...
async def _force_close_all(app, fetch_price_fn, moex_stocks):
    ...
    Учитывает и акции (MOEX_STOCKS), и фьючерсы (FUTURES)."""
    trades = load_trades()
...
async def fetch_russian_news(ticker: str = "", sector: str = "") -> list[dict]:
    ...
    for url, result in zip(RUSSIAN_NEWS_RSS, results):
        ...
        for item in result:
            ...
            # ── Ключевой фикс релевантности ──────────────────────────────
            # Раньше проверка is_specific срабатывала ТОЛЬКО для is_corporate.
            # Из-за этого секторные/общие паттерны (например "санкции против")
            # могли матчить заголовки которые формально содержат слова, но
            # вообще не про эту компанию/сектор. Пример реального бага:
            # "Каллас: ЕС не может ввести санкции против министра Израиля"
            # содержит "санкции против", получало вес -9 для ЛЮБОГО тикера
            # у которого "санкции" есть в search_words.
            #
            # Теперь: если новость классифицирована как факт (is_fact) с
            # ненулевым весом, но НЕ специфична для компании И не входит
            # в список действительно общерыночных тем (санкции на сектор,
            # ставка ЦБ) — обнуляем вес, оставляем только как фон.
            sector_wide_topics = {"санкции", "ставка", "цб", "опек", "нефть", "курс"}
            ...
            if cls["is_fact"] and cls["weight"] != 0 and not is_specific and not is_sector_relevant:
                # Новость матчит общее слово, но не про эту компанию и не про сектор —
                # обесцениваем, оставляем как информационный шум без влияния на скор.
            ...
            news_age_min = 0  # дефолт — если дата неизвестна считаем свежей
            try:
        ...
        for attempt in range(3):
            try:
                ...
                if "rate_limit" in err.lower() or "429" in err:
                    ...
                    time.sleep(wait)  # OK: вызывается в run_in_executor (отдельный поток)
                else:
                    logger.warning(f"Groq {GROQ_MODELS[i]}: {e}")
                    break  # не rate limit — пробуем следующую модель
    return ""
...
async def ai_check_relevance(title: str, ticker: str, company_name: str) -> bool:
    """
    Быстрая AI-проверка: эта новость действительно про указанную компанию,
    или просто содержит похожие слова в другом контексте?

    Решает проблему вида: заголовок "ЕС не может ввести санкции против
    министра Израиля" содержит слова "санкции против", но не имеет
    отношения к российской компании-тикеру.

    Используется только когда keyword-фильтр не может разрешить
    однозначно (нет прямого упоминания тикера/компании, но есть
    значимое ключевое слово с весом).
    """
    if not groq_client:
        return False  # без AI — по умолчанию считаем нерелевантной (безопаснее)
...
def find_support_resistance(df: pd.DataFrame, price: float, tf: str = "15m"):
    # Адаптивная глубина по таймфрейму
    depth = {"5m": 20, "15m": 20, "1h": 50, "4h": 150, "1d": 150}.get(tf, 20)
    r = df.tail(depth)
    highs, lows = [], []
    ...
    # Добавляем флипнутые уровни и сортируем по близости к цене
    supports    = sorted(flipped_to_support + raw_supports, key=lambda x: price - x)[:3]
    resistances = sorted(flipped_to_resistance + raw_resistances, key=lambda x: x - price)[:3]
...
def format_analysis(result: dict) -> str:
    ...
    # e-disclosure факты — приоритет над RSS
    ediscl = result.get("edisclosure_items", [])
    if ediscl:
        lines.append("📋 <b>e-disclosure (офиц. раскрытие):</b>")
        for it in ediscl[:3]:
            age   = it.get("age_min", 0)
            w     = it.get("weight", 0)
            w_e   = "🟢" if w > 2 else ("🔴" if w < -2 else "⚪")
            age_s = f"{age:.0f}м назад" if age < 60 else f"{age/60:.1f}ч назад"
            lines.append(f"{w_e} [{age_s}] {esc(it['title'][:100])}")
    ...
    elif not anomaly and not ediscl:
    ...
    if fact_news and not ediscl:
        # Показываем RSS факты только если нет e-disclosure
        lines += ["", "📌 <b>RSS факты:</b>"]
