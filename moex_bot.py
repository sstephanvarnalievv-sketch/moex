...
class _TinkoffRateLimiter:
    """
    Превентивный rate-limiter для запросов к Tinkoff API — token bucket,
    ограничивающий частоту запросов во времени. Раньше защита была только
    реактивной (retry после получения 429 в _analyze_with_semaphore) — тот
    же семафор ограничивает лишь ОДНОВРЕМЕННОСТЬ запросов (5 параллельно),
    а не их ЧАСТОТУ. При скане 30 тикеров, где каждый делает 6-7 запросов
    GetCandles (текущий ТФ + MTF 15m/1h/4h + дневные для HVN), расчётный
    пик — порядка 700+ запросов/минуту, что заметно выше официального
    лимита в 120 запросов/минуту на метод (T-Bank Dev Portal, лимитная
    политика). Держим RPM с запасом ниже официального лимита.
    """
    ...
    async def acquire(self):
        async with self._lock:
            now = time.time()
            # Оставляем только метки за последние 60 секунд
            self._timestamps = [t for t in self._timestamps if now - t < 60]
            if len(self._timestamps) >= self.max_per_minute:
                # Ждём пока самая старая метка не выйдет из окна в 60с
