...
# === 🔐 TLS: доверенный контекст = certifi + сертификаты Минцифры ===
# Т-Банк переводит T-API с сертификатов GlobalSign на сертификаты НУЦ Минцифры
# России (Russian Trusted CA). Эти сертификаты не входят в стандартный bundle
# certifi/системный trust-store за пределами РФ (в частности - на Railway),
# из-за чего запросы к invest-public-api.tinkoff.ru падали с
# SSLCertVerificationError "self-signed certificate in certificate chain".
# ssl=False (полное отключение проверки) НЕ используется намеренно: в
# заголовках запросов передаётся боевой TINKOFF_TOKEN, и отключение
# верификации сервера сделало бы бота уязвимым к MITM с перехватом токена.
# Вместо этого доверенный контекст включает И стандартный certifi (пока
# используется GlobalSign), И оба сертификата Минцифры (после переключения) -
# так что запросы проходят независимо от того, какой сертификат в данный
# момент отдаёт сервер Т-Банка.
def _build_trusted_ssl_context() -> ssl.SSLContext:
    if certifi is not None:
        ctx = ssl.create_default_context(cafile=certifi.where())
    else:
        ctx = ssl.create_default_context()
        logger.warning("Пакет certifi не установлен - используется системный trust-store")

    certs_dir = Path(__file__).resolve().parent / "certs"
    expected = ("russian_trusted_root_ca.pem", "russian_trusted_sub_ca.pem")
    loaded = []
    for fname in expected:
        ...
        if path.exists():
            ...
            logger.warning(
                f"Сертификат {fname} не найден в {certs_dir} - если Т-Банк уже "
                f"переключился на Минцифры, запросы к Tinkoff API будут падать "
                f"с SSLCertVerificationError. Скачай оба .pem с gosuslugi.ru/crt "
                f"и положи в папку certs/ рядом с ботом."
            )
    if loaded:
        logger.info(f"TLS: загружены доверенные сертификаты Минцифры: {', '.join(loaded)}")
    return ctx

_TRUSTED_SSL_CTX = _build_trusted_ssl_context()
...
MOEX_STOCKS = {
    ...
    "SFIN":  ("BBG00HGDC7N7", "ЭсЭфАй",               "финансы",
              {"сфи", "эсэфай", "sfin"}),
    "MFGS":  ("BBG004S68BR5", "Славнефть-Мегионнефтегаз", "нефтегаз",
              {"мегионнефтегаз", "мегион", "славнефть", "megionneftegaz"}),
    "RAGR":  ("BBG007N0Z367", "РусАгро",               "АПК",
              {"РусАгро", "Rusagro"}),
}
...
TRADE_MODES = {
    "low": {
        ...
        # 🟢 Раньше здесь были rsi_oversold/rsi_overbought/min_vol_ratio/
        # min_score/personality - ни одно из этих полей НИГДЕ не читалось,
        # реальные пороги брались только из get_instrument_profile(tier).
        # Режим влиял только на подпись в заголовке сообщения. Теперь режим
        # реально сдвигает пороги гейтов в compute_tech_score: LOW - строже
        # (сложнее пройти все 3 гейта), HARD - мягче (легче пройти).
        "trend_adj": 10,            # +10 к порогу min_trend (строже)
        "vol_percentile_adj": 10,   # +10 к порогу min_vol_percentile (строже)
        "exec_min_adj": 10,         # +10 к порогу exec_score (строже)
    },
    "mid": {
        "label": "🟡 MID (баланс)",
        "trend_adj": 0,
        "vol_percentile_adj": 0,
        "exec_min_adj": 0,
    },
    "hard": {
        "label": "🔴 HARD (агрессив)",
        "trend_adj": -10,           # -10 к порогу min_trend (легче пройти)
        "vol_percentile_adj": -15,  # -15 к порогу min_vol_percentile (легче пройти)
        "exec_min_adj": -10,        # -10 к порогу exec_score (легче пройти)
    },
}
...
class _TinkoffRateLimiter:
    ...
    def __init__(self, max_per_minute: int = 100):
        ...
        self._next_slot = 0.0
        self._lock = asyncio.Lock()  # 🟢 создаём сразу, а не лениво - без гонки при параллельных первых acquire()
...
class _AIRateLimiter:
    ...
    def __init__(self, max_per_minute: int = 10):
        self.min_interval = 60.0 / max_per_minute
        self._next_slot = 0.0
        self._lock = asyncio.Lock()  # 🟢 создаём сразу, а не лениво
...
def _gen_pending_key() -> str:
    """Уникальный короткий ключ для callback_data кнопки 'Войти'.

    Раньше ключ строился как f'{ticker}_{direction}_{timestamp}' и обрезался
    до 20 символов (а в части мест — вообще без timestamp, только
    ticker_direction). При двух сигналах по одному тикеру/направлению
    подряд это давало РЕАЛЬНУЮ коллизию: новая запись в _pending_trades
    перезатирала старую по тому же ключу, и кнопка на уже отправленном
    СТАРОМ сообщении при нажатии открывала сделку с параметрами (цена/SL/TP)
    от НОВОГО сигнала, а не от того, что видел пользователь на экране.
    uuid4 практически исключает коллизию и не требует кодировать в ключе
    ticker/direction/время - вся информация и так лежит в самом словаре."""
    return uuid.uuid4().hex[:16]

_http_session: aiohttp.ClientSession | None = None

def _get_http_session() -> aiohttp.ClientSession:
    """Возвращает единую HTTP-сессию с полной проверкой SSL через доверенный
    контекст (certifi + сертификаты Минцифры, см. _build_trusted_ssl_context).
    ВАЖНО: проверка сертификата НЕ отключается (ssl=False) - в запросах к
    Tinkoff передаётся боевой токен, отключение проверки открыло бы дорогу
    к MITM-перехвату токена."""
    global _http_session
    if _http_session is None or _http_session.closed:
        timeout = aiohttp.ClientTimeout(total=15)
        connector = aiohttp.TCPConnector(ssl=_TRUSTED_SSL_CTX)
...
RK_LAST_DECISION = "moex:last_decision"  # последний decision_log по тикеру - для /why

def _save_last_decision(ticker: str, decision_log: list[dict], filter_status: str) -> None:
    """Сохраняет последний decision_log по тикеру - чтобы /why <ticker> мог
    ответить 'почему бот принял такое решение' даже если сигнал ещё не
    закрылся (или не стал сделкой вообще)."""
    r = _get_redis()
    if not r:
        return
    try:
        payload = {"decision_log": decision_log, "filter_status": filter_status,
                   "ts": datetime.now(timezone.utc).isoformat(timespec="seconds")}
        r.hset(RK_LAST_DECISION, ticker.upper(), json.dumps(payload, ensure_ascii=False))
    except Exception as e:
        logger.warning(f"_save_last_decision {ticker}: {e}")

def _load_last_decision(ticker: str) -> dict | None:
    r = _get_redis()
    if not r:
        return None
    try:
        raw = r.hget(RK_LAST_DECISION, ticker.upper())
        return json.loads(raw) if raw else None
    except Exception as e:
        logger.warning(f"_load_last_decision {ticker}: {e}")
        return None
...
RK_TICKER_FACTS      = "moex:ticker_facts"  # факты по конкретной компании со сроком действия (дивиденды, байбек, допэмиссия и т.д.)
...
def _safe_redis_cleanup() -> None:
    ...
    # 🟢 Раньше здесь не было флага "уже выполнено" - этот блок безусловно
    # удалял ВСЕ секторальные модификаторы при КАЖДОМ старте бота (то есть
    # при каждом деплое), а не один раз как задумывалось миграцией. Из-за
    # этого выходной брифинг после любого редеплоя внезапно "терял" весь
    # накопленный за день секторальный фон, хотя данные были в порядке -
    # их просто стирали при старте до того, как брифинг успевал их прочитать.
    MIGRATION_FLAG_KEY = "moex:migrations:sector_modifiers_v2_cleared"
    try:
        if not r.exists(MIGRATION_FLAG_KEY):
...
BOT_SETTINGS_LABELS: dict[str, str] = {
    ...
    "news_dedup_similarity":    "Похожесть заголовков для дедупликации новостей, % (0-100)",
}
...
RK_FUTURES_SCAN_ENABLED = "moex:futures_scan_enabled"
_futures_scan_enabled_cache: bool | None = None

RK_GLOBAL_SCAN_MODE = "moex:global_scan_mode"
_global_scan_mode_cache: str | None = None

def get_scan_mode() -> str:
    """Режим (low/mid/hard) для АВТОМАТИЧЕСКОГО сканера (run_scanner_broadcast,
    run_futures_broadcast). Раньше эти функции были жёстко прописаны на
    TRADE_MODES['mid'] независимо от того, что выбрано в /mode - то есть
    выбор HARD никак не влиял на реальный поток сигналов автоскана, только
    на ручной /analyze. По умолчанию 'mid' (поведение не меняется, пока
    явно не переключишь)."""
    global _global_scan_mode_cache
    if _global_scan_mode_cache is not None:
        return _global_scan_mode_cache
    raw = _rget(RK_GLOBAL_SCAN_MODE)
    _global_scan_mode_cache = raw if raw in TRADE_MODES else "mid"
    return _global_scan_mode_cache
...
def set_scan_mode(mode: str) -> bool:
    ...
    if mode not in TRADE_MODES:
        return False
    ok = _rset(RK_GLOBAL_SCAN_MODE, mode)
    _global_scan_mode_cache = mode
    return ok

def get_futures_scan_enabled() -> bool:
    """Возвращает True если автоскан фьючерсов включен (по умолчанию True)."""
    global _futures_scan_enabled_cache
    if _futures_scan_enabled_cache is not None:
        return _futures_scan_enabled_cache
    raw = _rget(RK_FUTURES_SCAN_ENABLED)
    if raw is not None:
        _futures_scan_enabled_cache = raw.lower() in ("1", "true", "yes", "on")
    else:
        _futures_scan_enabled_cache = True
    return _futures_scan_enabled_cache

def set_futures_scan_enabled(enabled: bool) -> bool:
    """Включает/выключает автоскан фьючерсов (broadcast сигналов), не трогая
    сам вотчлист фьючерсов - удобно временно заглушить спам без удаления
    списка контрактов."""
    global _futures_scan_enabled_cache
    ok = _rset(RK_FUTURES_SCAN_ENABLED, "1" if enabled else "0")
    _futures_scan_enabled_cache = enabled
    return ok
...
# === 🔐 Атомарные async-обёртки над open_trade/close_trade ===
# open_trade/close_trade сами по себе синхронные (без await внутри), поэтому
# в текущем однопроцессном asyncio-приложении их read-modify-write уже
# выполняется без переключения на другие корутины. Обёртки через
# _TRADE_STORE_LOCK — это защита на будущее (если load/save когда-нибудь
# станут асинхронными, например при переходе на async-redis) и явная
# сериализация конкурентных вызовов из разных команд/фоновых циклов.
async def open_trade_safe(*args, **kwargs) -> str:
    async with _TRADE_STORE_LOCK:
        return open_trade(*args, **kwargs)

async def close_trade_safe(*args, **kwargs) -> dict | None:
    async with _TRADE_STORE_LOCK:
        return close_trade(*args, **kwargs)
...
TIME_EXIT_MIN_HOURS = 3  # минимальная "выдержка" сделки перед тем, как таймаут-выход может сработать
...
async def _check_trade(trade_id: str, t: dict, price: float, trades_cache: dict = None) -> str | None:
    ...
    if sl_hit:
        ...
        closed = await close_trade_safe(trade_id, "SL", price)
    ...
    if tp3_hit and status in ("open", "tp1_hit", "tp2_hit"):
        t["status"] = "closed"
        trades[trade_id]["status"] = "closed"
        closed = await close_trade_safe(trade_id, "TP3", price)
        pnl = closed.get("pnl_pct", 0) if closed else 0
        return (
    ...
    # 5. ТАЙМАУТ-ВЫХОД: сделка давно открыта, в минусе и без прогресса к TP1.
    # Раньше настройки time_exit_loss_threshold/time_exit_tp1_progress были
    # в /settings, но сама фича не была написана вообще - ничего не
    # проверяло ни возраст сделки, ни прогресс. Идея: если рынок за
    # TIME_EXIT_MIN_HOURS не подтвердил сетап (цена так и не продвинулась к
    # TP1), а результат хуже порога убытка - выходим сами, не дожидаясь SL,
    # чтобы не сидеть в застрявшей позиции "на надежде".
    if status == "open":
        try:
            opened_dt = datetime.strptime(t.get("opened_at", ""), "%Y-%m-%dT%H:%M:%S").replace(tzinfo=timezone.utc)
            elapsed_hours = (datetime.now(timezone.utc) - opened_dt).total_seconds() / 3600
        except Exception:
            elapsed_hours = 0

        if elapsed_hours >= TIME_EXIT_MIN_HOURS:
            pnl_pct = ((price - entry) / entry * 100) if is_long else ((entry - price) / entry * 100)
            tp1_distance = abs(tp1 - entry)
            if tp1_distance > 0:
                progress_pct = (((price - entry) / tp1_distance * 100) if is_long
                                else ((entry - price) / tp1_distance * 100))
            else:
                progress_pct = 0

            _settings      = get_bot_settings()
            loss_threshold = _settings["time_exit_loss_threshold"]
            min_progress   = _settings["time_exit_tp1_progress"]

            if pnl_pct <= loss_threshold and progress_pct < min_progress:
                t["status"] = "closed"
                trades[trade_id]["status"] = "closed"
                closed = await close_trade_safe(trade_id, "TIMEOUT", price)
                pnl = closed.get("pnl_pct", 0) if closed else pnl_pct
                return (
                    f"⏱ <b>Закрыто по таймауту - {ticker} {direction}</b>\n"
                    f"Цена: {price:,.2f} ₽ | В сделке: {elapsed_hours:.1f}ч\n"
                    f"📉 Результат: {pnl:+.2f}% (прогресс к TP1: {progress_pct:.0f}%)\n"
                    f"<i>Идея не подтвердилась за отведённое время - вышли сами, не дожидаясь SL.</i>"
                )
...
async def _force_close_all(app, fetch_price_fn, moex_stocks):
    ...
    for trade_id, t in open_trades.items():
        ...
        try:
            if should_close:
                closed = await close_trade_safe(trade_id, "EOD", price)
                pnl    = closed.get("pnl_pct", 0) if closed else pnl_pct
...
DEFAULT_WATCHLIST = [
    ...
    # 🟢 Расширение вотчлиста (было 29 тикеров - на активном рынке этого
    # мало для реальной частоты интрадей-сигналов). Добавлены ликвидные
    # бумаги, которые уже были определены в MOEX_STOCKS (сектор/tier есть),
    # но не входили в дефолтный скан вообще - в том числе T (Т-Банк),
    # который раньше даже не сканировался автоматически.
    "T", "SBERP", "VKCO", "ASTR", "HEAD", "CIAN", "X5", "LENT",
    "MDMG", "FIXR", "SVCB", "BSPB", "CBOM", "SGZH", "SMLT", "LSRG",
    "MTLR", "RASP", "ENPG", "TATNP", "SNGSP", "RTKMP", "OGKB", "MSNG",
    "TGKA", "FLOT",
]
...
async def fetch_imoex_index_candles(days: int = 1, interval: int = 60) -> pd.DataFrame | None:
    """
    Настоящие свечи индекса IMOEX через MOEX ISS.

    Раньше вместо индекса использовалась акция самой Мосбиржи (тикер "MOEX",
    FIGI BBG004S68507) как "прокси рынка" - но это акция конкретной
    компании-биржи, её цена зависит от прибыли биржи (комиссии, объёмы
    торгов), а не от рынка в целом. У Tinkoff API нет нормальных свечей по
    индексам, поэтому берём напрямую с MOEX ISS - там индекс IMOEX отдаётся
    честно, без прокси.

    interval (код MOEX ISS): 1=1мин, 10=10мин, 60=1час, 24=1день.
    """
    cache_key = f"imoex_iss_{interval}_{days}"
    now = time.time()
    ...
    try:
        ...
        params = {"from": frm.strftime("%Y-%m-%d"), "till": till.strftime("%Y-%m-%d"), "interval": interval}
        async with session.get(url, params=params) as r:
            if r.status != 200:
                logger.warning(f"fetch_imoex_index_candles: HTTP {r.status}")
                return None
            data = await r.json()
        ...
        rename = {"begin": "timestamp"}
        df = df.rename(columns=rename)
        for c in ("open", "high", "low", "close"):
            if c in df.columns:
                df[c] = pd.to_numeric(df[c], errors="coerce")
        df["timestamp"] = pd.to_datetime(df["timestamp"])
        keep = [c for c in ("timestamp", "open", "high", "low", "close") if c in df.columns]
        df = df[keep].dropna()
        if df.empty:
            return None

        _cache[cache_key] = {"val": df, "ts": now}
        return df
    except Exception as e:
        logger.warning(f"fetch_imoex_index_candles: {e}")
        return None
...
async def fetch_imoex_regime() -> dict:
    ...
    if cache_key in _cache and now - _cache[cache_key]["ts"] < 1200:
        return _cache[cache_key]["val"]
    try:
        df = await fetch_imoex_index_candles(days=250, interval=24)  # дневные свечи, ~250 торговых дней
        if df is None or len(df) < 30:
            raise ValueError("Недостаточно данных для IMOEX")
...
def get_cbr_dates_2026() -> list:
    """Заседания ЦБ РФ 2026-2027 (Ключевая ставка)."""
    cbr_dates = [
        "2026-02-13", "2026-03-20", "2026-04-24", "2026-06-05",
        "2026-07-24", # <-- ЗАСЕДАНИЕ ЦБ В ЭТУ ПЯТНИЦУ!
        "2026-09-11", "2026-10-23", "2026-12-18",
        "2027-02-12", "2027-03-19", "2027-04-23", "2027-06-04",
        "2027-07-23", "2027-09-10", "2027-10-22", "2027-12-17",
    ]
    events = []
    now = datetime.now(timezone.utc)
    for d in cbr_dates:
        try:
            # Решение по ставке объявляется в 13:30 МСК (10:30 UTC)
            ev_dt = datetime.strptime(d, "%Y-%m-%d").replace(hour=10, minute=30, tzinfo=timezone.utc)
            if ev_dt < now - timedelta(days=1):
                continue
            events.append({
                ...
                "type":         "cbr_rate",
                "source":       "cbr",
                "note":         "Волатильность аномальная. Не торговать за 2ч до и 1ч после!",
            })
        except Exception:
            continue
    return events
...
# 🟢 Срок действия факта в днях по типу события. Раньше ЛЮБАЯ новость,
# включая "допэмиссия"/"отмена дивидендов", выпадала из анализа через
# фиксированные NEWS_MAX_AGE_MINUTES (120 мин, а для ликвидных бумаг -
# всего 10 мин), независимо от важности. Допэмиссия/отмена дивидендов -
# это события, которые реально давят на сентимент бумаги ещё много дней,
# а не 10 минут. Здесь событие само определяет, сколько дней оно ещё
# должно учитываться в анализе (см. _save_ticker_fact/_load_ticker_facts).
EVENT_DURATION_DAYS = {
    "отмена дивидендов":        5,
    "рекомендация дивидендов":  5,
    "дивиденды объявлены":      5,
    "дивиденды выше ожиданий":  5,
    "байбек":                   7,
    "допэмиссия":               7,
    "SPO":                      7,
    "рекордная прибыль":        3,
...
EVENT_DURATION_DEFAULT_DAYS = 1  # для событий не из таблицы выше - минимальный запас

def _save_ticker_fact(ticker: str, event: str, weight: int, title: str) -> None:
    """Сохраняет факт по конкретной компании со сроком действия в Redis -
    чтобы значимое событие (дивиденды/байбек/допэмиссия/санкции) продолжало
    учитываться в анализе дни после того, как сама RSS-новость выпала из
    короткого окна свежести NEWS_MAX_AGE_MINUTES."""
    r = _get_redis()
    if not r or weight == 0:
        return
    days = EVENT_DURATION_DAYS.get(event, EVENT_DURATION_DEFAULT_DAYS)
    try:
        fact = {
            "event": event, "weight": weight, "title": title[:200],
            "set_at": time.time(), "expires_at": time.time() + days * 86400,
        }
        field = f"{ticker.upper()}:{event}"
        r.hset(RK_TICKER_FACTS, field, json.dumps(fact, ensure_ascii=False))
    except Exception as e:
        logger.warning(f"_save_ticker_fact {ticker}/{event}: {e}")

def _load_ticker_facts(ticker: str) -> list[dict]:
    """Читает ещё активные (не протухшие по expires_at) факты по тикеру.
    Отдаёт список в формате, совместимом с обычными news_items, чтобы
    напрямую подмешиваться в all_facts / промпт для ИИ."""
    r = _get_redis()
    if not r:
        return []
    try:
        raw_map = r.hgetall(RK_TICKER_FACTS)
    except Exception as e:
        logger.warning(f"_load_ticker_facts hgetall: {e}")
        return []
    ...
    for field, raw in raw_map.items():
        ...
        try:
            fact = json.loads(raw)
            if fact.get("expires_at", 0) > now:
                age_min = (now - fact.get("set_at", now)) / 60
                result.append({
                    "title": f"[длится {age_min/1440:.1f}д] {fact['title']}",
                    "event": fact["event"], "weight": fact["weight"],
                    "effective_weight": fact["weight"],
                    "is_fact": True, "is_opinion": False, "is_corporate": True,
                    "is_specific": True, "age_min": round(age_min, 0),
                    "source": "sustained_fact",
                })
            else:
                expired_keys.append(field)
        except Exception:
            expired_keys.append(field)

    if expired_keys:
        try:
            r.hdel(RK_TICKER_FACTS, *expired_keys)
        except Exception:
            pass
...
def _dedup_news_items(raw: list[dict], threshold: float = None) -> list[dict]:
    """
    Дедупликация новостей по смысловой похожести заголовков (SequenceMatcher),
    а не по точному совпадению первых N символов.

    Раньше два заголовка про одно и то же событие, но с разной первой фразой
    ("Совет директоров рекомендовал дивиденды..." vs "Стало известно, что
    рекомендованы дивиденды...") считались РАЗНЫМИ новостями, потому что не
    совпадали префиксом - обе попадали в анализ как два независимых факта,
    задваивая вес события и дублируя контекст в промпте для ИИ.

    Сравниваем каждую новость (от свежих к старым) с уже принятыми - если
    похожесть заголовков выше порога (по умолчанию берётся из настройки
    news_dedup_similarity, 0-100%), считаем дублем и оставляем только более
    свежую версию.
    """
    if not raw:
        return []
    if threshold is None:
        threshold = get_bot_settings()["news_dedup_similarity"] / 100.0

    raw_sorted = sorted(raw, key=lambda x: x.get("age_min", 999))
    unique: list[dict] = []
    seen_norms: list[str] = []
    for it in raw_sorted:
        norm = re.sub(r'[^\w\s]', '', it.get("title", "").lower()).strip()
        if not norm:
            continue
        if any(SequenceMatcher(None, norm, prev).ratio() >= threshold for prev in seen_norms):
            continue
        unique.append(it)
...
async def fetch_russian_news(ticker: str = "", sector: str = "") -> list[dict]:
    ...
    for url, result in zip(RUSSIAN_NEWS_RSS, results):
        ...
        for item in result:
            ...
            # 🟢 Раньше военные стоп-слова резали ЛЮБОЙ заголовок безусловно,
            # даже если в нём явно упоминается сама компания. Из-за этого
            # новости вида "Склад Ozon подвергся атаке дрона" отбрасывались
            # тут же ("дрон"/"взрыв"/"обстрел" в списке стоп-слов) - хотя
            # атака на инфраструктуру САМОЙ компании это ровно то, что этот
            # источник должен ловить, а не общий репортаж с фронта, для
            # отсева которого стоп-слова и придуманы. Теперь стоп-слова не
            # режут заголовок, если в нём есть упоминание именно этой компании.
            mentions_company = bool(specific_words) and any(w in title_lower for w in specific_words)
            if any(sw in title_lower for sw in MILITARY_GEO_STOPWORDS) and not mentions_company:
                continue
            ...
            # 🟢 Раньше is_sector_relevant считался ПОСЛЕ этой проверки, и
            # новость, которая совпала только по секторному слову (не по
            # тикеру/названию компании), выбрасывалась здесь безусловно -
            # логика ниже (is_sector_relevant), написанная специально для
            # "не про компанию, но про её сектор - не выбрасывать, просто
            # понизить вес", была физически недостижима для таких новостей.
            # Пример: "спрос на золото в РФ упал" для PLZL - совпадает
            # только по слову "золото" (не по "Полюс"/тикеру), раньше
            # отбрасывалось здесь и ИИ его вообще не видел.
            ...
            if only_generic_matched and not is_specific and not is_sector_relevant:
                continue
            ...
            # 🟢 Значимый факт именно про эту компанию (не сектор, не шум) -
            # сохраняем со сроком действия из EVENT_DURATION_DAYS, чтобы он
            # продолжал учитываться в анализе после того, как сама новость
            # выпадет из короткого окна NEWS_MAX_AGE_MINUTES.
            if ticker and is_specific and cls["is_fact"] and cls["weight"] != 0:
                _save_ticker_fact(ticker, cls["event"], cls["weight"], title)
    ...
    # Быстрая точная проверка по event_key (ticker+тип события) - ловит явные
    # повторы того же классифицированного события мгновенно, без сравнения
    # текста. Затем похожесть заголовков ловит перефразированные дубли,
    # которые event_key не поймал (разные event либо event вообще не задан).
    seen_events, deduped_by_event = set(), []
    for it in raw_items:
        ...
        deduped_by_event.append(it)
    unique = _dedup_news_items(deduped_by_event)
...
async def fetch_macro_news() -> list[dict]:
    ...
    санкции общего характера, глобальные геополитические события (MACRO_KEYWORDS).

    ВАЖНО: раньше эта функция была алиасом на fetch_russian_news() без
    тикера и без сектора - то есть искала по ВСЕМ компаниям сразу общими
    словами типа "дивиденд"/"убыток"/"прибыль", и то, что находилось про
    СОВСЕМ ДРУГУЮ компанию, помечалось как "макро" и утекало в ИИ-промпт
    для текущего анализируемого тикера. Теперь фильтруется строго по
    MACRO_KEYWORDS (ставка ЦБ, рубль, санкции, ФРС/ЕЦБ) - без привязки к
    конкретной компании, поэтому и не может быть спутана с конкретной акцией.
    """
...
async def fetch_sector_news(sector: str) -> list[dict]:
    ...
    for url, result in zip(RUSSIAN_NEWS_RSS, results):
        ...
        for item in result:
            ...
            title_lower = title.lower()
            # 🟢 Та же логика, что в fetch_russian_news: атака на инфраструктуру
            # СЕКТОРА (склад, завод, НПЗ) - это ровно то, что этот источник
            # должен ловить для секторальных модификаторов, а не общий
            # репортаж с фронта. Не режем, если заголовок совпадает по
            # ключевым словам этого сектора.
            full_check = (title_lower + " " + item.get("desc", "").lower())
            mentions_sector = any(kw in full_check for kw in keywords)
            if any(sw in title_lower for sw in MILITARY_GEO_STOPWORDS) and not mentions_sector:
                continue
            if any(sw in title_lower for sw in OFFTOPIC_STOPWORDS):
                continue
...
_last_weekend_topic_ts: dict[str, float] = {}
WEEKEND_TOPIC_COOLDOWN_SEC = 3600  # один и тот же повод (сектор/новость/компания) - не чаще раза в час

async def run_weekend_briefing(app):
    """
    По выходным MOEX торгует только в рамках доп.сессии выходного дня
    (низкая ликвидность, цена почти не двигается), но новости, гео-события
    и товарные рынки продолжают жить своей жизнью - а в понедельник на
    основной сессии всё это может резко ударить по открытию.

    run_geopolitical_scan уже работает 24/7 (без привязки к торговому дню)
    и обновляет секторальные модификаторы по свежим commodity/macro/
    секторным новостям - поэтому здесь не нужно заново лезть в RSS, только
    собрать то, что уже накопилось в Redis, в читаемый брифинг.

    ЭТО НЕ СИГНАЛ. Без SL/TP, без кнопки "войти" - чистая рекомендация
    "на что обратить внимание перед открытием понедельника", решение о
    входе (в том числе на доп.сессии выходного дня, если бумага туда
    допущена) остаётся полностью за пользователем.
    """
    global _last_weekend_topic_ts
    weekday, hour, _ = get_msk_time()
    if weekday not in (5, 6):  # только суббота/воскресенье
        return
    if not (9 <= hour < 22):  # раньше слал и в 2-3 ночи, разумных часов не было вообще
        return

    mods = _load_sector_modifiers()
    strong_mods = {s: m for s, m in mods.items() if abs(m.get("strength", 0)) >= 6}

    try:
        macro_news = [it for it in await fetch_macro_news() if it.get("age_min", 999) <= 360]
    except Exception as e:
        logger.debug(f"run_weekend_briefing macro: {e}")
        macro_news = []

    # 🟢 Раньше по выходным сканировался только сектор/макро/сырьё
    # (run_geopolitical_scan работает 24/7), а конкретные компании из
    # вотчлиста - никогда: run_news_driven_scan, который следит именно за
    # новостями по тикерам (дивиденды/отчёты/допэмиссия), жёстко завязан на
    # is_trading_day и по выходным не запускается вообще. А именно на
    # выходных компании чаще всего и объявляют такое - когда рынок закрыт
    # и не может сразу отреагировать. Добавляем отдельный лёгкий обход
    # вотчлиста здесь, независимо от торгового дня.
    company_facts: list[tuple[str, dict]] = []
    try:
        wl = load_watchlist()
    except Exception:
        wl = []
    if wl:
        async def _fetch_company_news(t: str):
            try:
                sector_t = MOEX_STOCKS.get(t, ("", "", ""))[2]
                items = await fetch_russian_news(t, sector_t)  # уже сохраняет sustained facts как побочный эффект
                strong = [it for it in items
                          if it.get("is_fact") and it.get("is_specific") and not it.get("is_opinion")
                          and abs(it.get("weight", 0)) >= 6 and it.get("age_min", 999) <= 360]
                return t, strong
            except Exception as e:
                logger.debug(f"run_weekend_briefing company news {t}: {e}")
                return t, []

        try:
            results = await asyncio.wait_for(
                asyncio.gather(*[_fetch_company_news(t) for t in wl], return_exceptions=True),
                timeout=60.0
            )
            for res in results:
                ...
                t, strong = res
                for it in strong:
                    company_facts.append((t, it))
        except asyncio.TimeoutError:
            logger.warning("run_weekend_briefing: таймаут скана компаний (60с)")

    if not strong_mods and not macro_news and not company_facts:
        return

    # 🟢 Раньше ключ дедупа включал текст m['reason'] - а он от AI-классификатора
    # (см. classify_geopolitical_impact), который мог формулировать чуть
    # иначе один и тот же повод при каждой переклассификации (см. фикс
    # _already_classified выше) - из-за этого дедуп почти никогда не
    # срабатывал, и весь брифинг слался заново каждый час с той же сутью,
    # но другими словами. Теперь ключ - только сектор/заголовок/компания,
    # без изменчивого текста причины, плюс cooldown по каждой теме отдельно
    # (не по всему сообщению целиком).
    now = time.time()
    topic_keys = {f"sector_{s}" for s in strong_mods}
    topic_keys |= {f"macro_{it['title'][:60]}" for it in macro_news}
    topic_keys |= {f"company_{t}_{it.get('event', it['title'][:40])}" for t, it in company_facts}

    new_topics = {k for k in topic_keys if now - _last_weekend_topic_ts.get(k, 0) >= WEEKEND_TOPIC_COOLDOWN_SEC}
    if not new_topics:
        return
    for k in topic_keys:
        _last_weekend_topic_ts[k] = now
    if len(_last_weekend_topic_ts) > 300:
        trimmed = sorted(_last_weekend_topic_ts.items(), key=lambda x: -x[1])[:150]
        _last_weekend_topic_ts = dict(trimmed)

    lines = [
        "📅 <b>ВЫХОДНОЙ БРИФИНГ - не сигнал, к размышлению</b>",
        "<i>Рынок почти не движется, но новостной фон копится к понедельнику</i>",
        "",
    ]

    if company_facts:
        lines.append("🏢 <b>Новости по компаниям из вотчлиста:</b>")
        for t, it in sorted(company_facts, key=lambda x: -abs(x[1].get("weight", 0)))[:8]:
            name = MOEX_STOCKS.get(t, ("", t, ""))[1]
            lines.append(f"   • <b>{esc(t)}</b> ({esc(name)}): {esc(it['title'][:130])} ({it.get('weight', 0):+d}/10)")
        lines.append("")

    if strong_mods:
        lines.append("🌍 <b>Активный секторальный фон:</b>")
        for sector, mod in sorted(strong_mods.items(), key=lambda x: -abs(x[1].get("strength", 0))):
            direction = mod.get("direction", "")
            arrow = ("🟢 ПОЗИТИВ" if direction == "bullish" else
                     "🔴 НЕГАТИВ" if direction == "bearish" else "⚪")
            tickers_in_sector = [tk for tk, v in MOEX_STOCKS.items() if v[2] == sector][:4]
            lines.append(f"{arrow} <b>{esc(sector)}</b> (сила {mod.get('strength', 0):+d}/10)")
            lines.append(f"   <i>{esc(str(mod.get('reason', ''))[:150])}</i>")
            if tickers_in_sector:
                lines.append(f"   Бумаги сектора: {', '.join(esc(t) for t in tickers_in_sector)}")
        lines.append("")

    if macro_news:
        lines.append("📰 <b>Свежий общий макрофон:</b>")
        for it in macro_news[:4]:
            lines.append(f"   • {esc(it['title'][:140])} <i>({it.get('age_min', 0):.0f} мин назад)</i>")
        lines.append("")

    lines.append(
        "⚠️ Это ИНФОРМАЦИЯ, а не торговый сигнал. На доп.сессии выходного дня "
        "торгуется ограниченный список бумаг, ликвидность низкая, спреды широкие - "
        "решение и риск полностью на тебе."
    )
    ...
    for chat_id in SCANNER_CHAT_IDS:
        try:
            ...
            logger.warning(f"Weekend briefing failed for {chat_id}: {e}")

async def run_weekend_briefing_loop(app):
    while True:
        try:
            await run_weekend_briefing(app)
        except Exception as e:
            logger.warning(f"run_weekend_briefing_loop: {e}")
        await asyncio.sleep(3600)  # раз в час - геомодификаторы обновляются раз в 30 мин, чаще не нужно

_classified_headlines_ts: dict[str, float] = {}
CLASSIFIED_HEADLINE_TTL_SEC = 3600  # не переклассифицировать тот же заголовок чаще раза в час

def _already_classified(title: str) -> bool:
    """
    Раньше один и тот же заголовок (пока age_min<=60) переклассифицировался
    заново каждые 30 минут - LLM не полностью детерминирован, поэтому
    "reason" для того же события каждый раз выходил с чуть другой
    формулировкой, и _set_sector_modifier перезаписывал модификатор новым
    текстом. Это ломало дедуп в выходном брифинге (он был завязан на текст
    reason) и просто жгло квоту ИИ на переклассификацию того, что уже
    классифицировано. Теперь один заголовок классифицируется не чаще раза
    в час, независимо от того, сколько раз цикл его увидит за это время.
    """
    now = time.time()
    key = re.sub(r'[^\w\s]', '', title.lower()).strip()[:80]
    last = _classified_headlines_ts.get(key, 0)
    if now - last < CLASSIFIED_HEADLINE_TTL_SEC:
        return True
    _classified_headlines_ts[key] = now
    if len(_classified_headlines_ts) > 500:
        cutoff = now - CLASSIFIED_HEADLINE_TTL_SEC * 2
        for k in [k for k, v in _classified_headlines_ts.items() if v < cutoff]:
            del _classified_headlines_ts[k]
    return False
            ...
            for sector in COMMODITY_SECTORS:
                ...
                for item in news[:2]:  
                    if item.get("age_min", 999) > 60:  
                        continue
                    if _already_classified(item["title"]):
                        continue
                    try:
...
_macro_snapshot_cache: dict = {}
_macro_snapshot_ts: float = 0.0

async def get_macro_snapshot() -> dict:
    """Общий макро-фон (USD/RUB, нефть Brent, золото) с изменением за ~1 час.
    Кэшируется на 5 минут - фон одинаковый для всех тикеров в одном цикле
    скана, нет смысла дёргать Tinkoff по 3 запроса на каждый анализ."""
    global _macro_snapshot_cache, _macro_snapshot_ts
    now = time.time()
    if _macro_snapshot_cache and now - _macro_snapshot_ts < 300:
        return _macro_snapshot_cache

    snapshot = {"usd_rub": None, "usd_rub_chg_pct": None,
                "brent": None, "brent_chg_pct": None,
                "gold": None, "gold_chg_pct": None}

    try:
        usd_rub = await _get_usd_rub()
        if usd_rub:
            snapshot["usd_rub"] = round(usd_rub, 2)
    except Exception as e:
        logger.debug(f"macro_snapshot usd_rub: {e}")

    async def _driver_change(code: str) -> tuple[float | None, float | None]:
        ...
        if not figi or (figi.startswith("FUT") and len(figi) <= 15):
            return None, None
        try:
            df_d = await fetch_candles_tinkoff(figi, "CANDLE_INTERVAL_15_MIN", 20)
            if df_d is None or len(df_d) < 5:
                return None, None
            last = float(df_d["close"].iloc[-1])
            past = float(df_d["close"].iloc[-5])
            chg = (last - past) / past * 100 if past else None
            return round(last, 2), (round(chg, 2) if chg is not None else None)
        except Exception:
            return None, None

    try:
        brent_last, brent_chg = await _driver_change("BRU6")
        snapshot["brent"] = brent_last
        snapshot["brent_chg_pct"] = brent_chg
    except Exception as e:
        logger.debug(f"macro_snapshot brent: {e}")

    try:
        gold_last, gold_chg = await _driver_change("GDU6")
        snapshot["gold"] = gold_last
        snapshot["gold_chg_pct"] = gold_chg
    except Exception as e:
        logger.debug(f"macro_snapshot gold: {e}")

    _macro_snapshot_cache = snapshot
    _macro_snapshot_ts = now
    return snapshot
...
async def ai_evaluate_news(news_items: list[dict], ticker: str, sector: str,
                           ...
                           htf_trend: dict = None,
                           cal_check: dict = None,
                           div_check: dict = None) -> dict:
    """
    ПОЛНАЯ AI ОЦЕНКА новостного и макро-фона.

    В отличие от старой версии, ИИ теперь получает не только пред-отфильтрованные
    по ключевым словам "факты", а весь собранный контекст: свежие новости по
    компании/сектору/товарным рынкам/макро (без предварительной фильтрации),
    дивидендный календарь и отсечки, ближайшие события календаря (ЦБ, отчёты),
    и текущий макро-фон (USD/RUB, Brent, золото) с изменением за последний час.

    ИИ отдаёт JSON с confidence (0-100%) и weight (-10..10) - насколько фон
    подтверждает/противоречит тех. сигналу. ИИ НЕ может выставить BLOCKED -
    жёсткая блокировка (дивиденды, отмены) остаётся на детерминированных
    правилах ниже (blocking_found), чтобы гарантированные механические
    события (гэп на отсечке) не зависели от того, "поверил" ли в них ИИ.
    """
    if not get_ai_enabled():
    ...
    # 🟢 sustained_fact (дивиденды/байбек/допэмиссия и т.д., см. EVENT_DURATION_DAYS)
    # намеренно "старше" NEWS_MAX_AGE_MINUTES - у них своя длительность
    # действия, поэтому обычный фильтр свежести к ним не применяется.
    rss_facts = [it for it in (news_items or [])
                 if it.get("is_fact") and not it.get("is_opinion")
                 and (it.get("source") == "sustained_fact" or it.get("age_min", 999) <= NEWS_MAX_AGE_MINUTES)]
    ...
    # 🟢 Раньше ИИ вызывался только если keyword-фильтр уже нашёл "факт" и
    # (была аномалия цены ИЛИ раскрытие информации ИЛИ вес выше порога) -
    # то есть почти всегда пропускался, и дивиденды/ЦБ/нефть/золото/доллар
    # в промпт вообще не попадали. Теперь ИИ вызывается на каждый сигнал,
    # если включён и есть хотя бы один провайдер (Gemini/Groq).
    should_call_ai = bool(gemini_model or groq_client)

    if should_call_ai:
        company_name = MOEX_STOCKS.get(ticker, ("", ticker, ""))[1]

        # 🟢 Явно помечаем SCOPE каждой новости - раньше промпт содержал
        # голый список заголовков без указания, к чему они относятся, и ИИ
        # мог принять секторальную/товарную/общерыночную новость за новость
        # именно об этой компании ("прикручивал не туда"). Теперь каждая
        # строка помечена, откуда она и какой у неё охват - у ИИ нет
        # возможности спутать источник, только неверно его использовать.
        def _scope_label(it: dict) -> str:
            if it.get("source") == "calendar":
                return "КАЛЕНДАРЬ"
            if it.get("source") == "dividends":
                return "ДИВИДЕНДЫ"
            if it.get("is_macro"):
                return "МАКРО РФ/МИР (не про конкретную компанию)"
            if it.get("is_commodity"):
                return "ТОВАРНЫЙ РЫНОК (не про конкретную компанию)"
            if it.get("is_sector_news"):
                return f"СЕКТОР {sector} (не факт про именно эту компанию)"
            if it.get("is_specific"):
                return f"КОМПАНИЯ {ticker}"
            return f"ОБЩЕЕ ПО РЫНКУ (возможно не про {ticker})"

        # Топ-8 самых свежих новостей из ВСЕХ источников (компания, сектор,
        # товарные рынки, макро) - без предварительной обрезки по keyword-фильтру,
        # чтобы ИИ видел полную картину, а не только то, что совпало с шаблонами.
        # 🟢 Раньше топ-8 выбирались только по свежести (age_min), из-за чего
        # реальный факт про саму компанию мог вытесняться потоком свежего,
        # но менее важного фона (сектор/товар/макро). Теперь сначала идут
        # факты именно про компанию и календарь/дивиденды (приоритет 0),
        # затем сектор/товарный рынок (1), затем общий макро-фон (2) -
        # и только внутри каждой группы сортировка по свежести.
        def _scope_priority(it: dict) -> int:
            if it.get("source") in ("calendar", "dividends") or it.get("is_specific"):
                return 0
            if it.get("is_sector_news") or it.get("is_commodity"):
                return 1
            if it.get("is_macro"):
                return 2
            return 1
        ...
        sorted_news = sorted(
            news_items or [],
            key=lambda x: (_scope_priority(x), x.get("age_min", 999)),
        )[:8]
        if sorted_news:
            news_text = "\n".join(
                f"- [{_scope_label(it)}, {it.get('age_min', 0):.0f}мин назад] {it.get('title', '')}"
                for it in sorted_news
            )
        else:
            news_text = "(свежих новостей не найдено)"

        calendar_lines = []
        if cal_check and cal_check.get("warning"):
            calendar_lines.append(f"Календарь событий (ЦБ/отчёты/санкции): {cal_check['warning']}")
        if div_check and div_check.get("dividend_info"):
            calendar_lines.append(f"Дивиденды: {div_check['dividend_info']}")
        calendar_text = "\n".join(calendar_lines) if calendar_lines else "(значимых календарных событий нет)"

        macro_text = "(макро-данные недоступны)"
        try:
        ...
        if macro_snapshot:
            parts = []
            if macro_snapshot.get("usd_rub") is not None:
                chg = macro_snapshot.get("usd_rub_chg_pct")
                parts.append(f"USD/RUB {macro_snapshot['usd_rub']}" + (f" ({chg:+.2f}% за час)" if chg is not None else ""))
            if macro_snapshot.get("brent") is not None:
                chg = macro_snapshot.get("brent_chg_pct")
                parts.append(f"Нефть Brent {macro_snapshot['brent']}" + (f" ({chg:+.2f}% за час)" if chg is not None else ""))
            if macro_snapshot.get("gold") is not None:
                chg = macro_snapshot.get("gold_chg_pct")
                parts.append(f"Золото {macro_snapshot['gold']}" + (f" ({chg:+.2f}% за час)" if chg is not None else ""))
            if parts:
                macro_text = "; ".join(parts)

        prompt = f"""Ты - строгий финансовый аналитик МосБиржи (MOEX).

Компания: {company_name} ({ticker}), профильный сектор: {sector}
Технический сигнал: {tech_signal} (score {tech_score})

Свежие новости (компания/сектор/товарные рынки/макро):
{news_text}

Календарь и дивиденды:
{calendar_text}
...
Задание:
Каждая новость помечена своим охватом в квадратных скобках: КОМПАНИЯ {ticker} -
факт именно про эту компанию; СЕКТОР/ТОВАРНЫЙ РЫНОК/МАКРО - общий фон, который
может влиять на компанию косвенно, но НЕ является событием, произошедшим с
именно этой компанией. ВАЖНО: никогда не пиши в summary, что новость с меткой
СЕКТОР/ТОВАРНЫЙ РЫНОК/МАКРО произошла "с {company_name}" или "у {ticker}" -
такие новости только про общий фон отрасли/рынка, а не про саму компанию.
Если единственная информация - это общий фон без событий именно по компании,
confidence не должен быть высоким просто потому что новость выглядит важной -
она должна быть релевантна именно {company_name}, а не рынку в целом.

Оцени, насколько ВСЯ эта картина (новости + календарь + дивиденды + макро-фон)
ПОДТВЕРЖДАЕТ или ПРОТИВОРЕЧИТ техническому сигналу {tech_signal} по {company_name}.
Учитывай новости не только про саму компанию, но и про её сектор, товарные
рынки (нефть/газ/золото - если компания сырьевая) и общий макро-фон (курс
рубля, ставка ЦБ), если они значимы для этой конкретной бумаги.
Не пытайся полностью заблокировать сигнал - только оцени силу подтверждения
(жёсткая блокировка по дивидендам/отменам уже выполняется отдельно, до тебя).

ВАЖНО про горизонт: эта сделка держится ВНУТРИ ДНЯ, максимум 5-10 часов, не
дольше. Определи, когда новость реально повлияет на цену:
- immediate: эффект уже происходит или произойдёт в ближайший час
- intraday: эффект развернётся в течение сегодняшней сессии (1-8 часов)
- swing: эффект реальный, но раскроется за несколько дней, не сегодня
- background: общий информационный фон без конкретного момента влияния на
  цену (например "Германия рассматривает переход на геотермальную энергию
  к 2050 году" - это НЕ должно двигать вес сегодняшней сделки по газовой
  компании, даже если тема формально релевантна сектору)
Если horizon = "background" или "swing" - вес должен быть близок к 0 или
умеренным, а не полным, ПОСКОЛЬКУ ЭТА СДЕЛКА ЗАКРОЕТСЯ СЕГОДНЯ и не
доживёт до момента, когда такая новость реально что-то изменит.

Ответь СТРОГО в формате JSON:
{{
  "confidence": число от 0 до 100 (насколько фон релевантен и достоверен),
  "weight": число от -10 до 10 (насколько фон ПОДТВЕРЖДАЕТ (+) или ПРОТИВОРЕЧИТ (-) сигналу),
  "horizon": "immediate" | "intraday" | "swing" | "background",
  "explains_movement": true/false,
  "summary": "одно короткое предложение на русском, что решило дело"
}}"""
        ...
        try:
            ...
            if m:
                ...
                horizon = str(obj.get("horizon", "intraday")).strip().lower()
                if horizon not in ("immediate", "intraday", "swing", "background"):
                    horizon = "intraday"
                ...
                # 🟢 Горизонт: сделка живёт максимум 5-10 часов, поэтому новость,
                # эффект которой раскроется через дни (swing) или вообще не имеет
                # конкретного момента влияния (background), не должна двигать вес
                # так же сильно, как то, что реально повлияет сегодня. Раньше ИИ
                # мог дать вес -6 на новость про энергопереход Германии к 2050
                # году просто потому что тема формально "про сектор".
                horizon_mult = {"immediate": 1.0, "intraday": 1.0, "swing": 0.4, "background": 0.0}[horizon]
                if horizon_mult < 1.0 and event_weight != 0:
                    old_weight = event_weight
                    event_weight = int(round(event_weight * horizon_mult))
                    if horizon == "background":
                        llm_summary = f"{llm_summary} [фон без горизонта влияния - вес занулён]".strip()
                    elif old_weight != event_weight:
                        llm_summary = f"{llm_summary} [horizon={horizon}, вес снижен]".strip()
    ...
    status_map = {
        ...
        # 🟢 Раньше отсутствовал в карте -> проваливался в tech_signal и
        # незаметно пропадал. NEWS_ONLY - это оповещение без техники, поэтому
        # направление берём из знака веса, а не из is_long/is_short (их нет).
        "NEWS_ONLY": ("📢 LONG (по новостям, без техники)" if final_weight > 0
                      else "📢 SHORT (по новостям, без техники)"),
    }
    ...
    return {
        ...
        "confidence": confidence, "confirmed": confirmed, "horizon": horizon,
...
def calculate_sl_tp_stocks(signal: str, price: float, atr: float,
    ...
    if is_long:
        ...
        if len(tp_candidates) >= 3:
            ...
            # 🟢 Было risk×1.5 - по данным /outcome_stats средний MFE (лучшая
            # точка движения) по всем сигналам был всего +0.60%, а TP1×1.5
            # требовал минимум ~1.5% при минимальном риске 1% (часто больше,
            # если риск шире) - большинство сигналов физически не долетали
            # даже до TP1 и "протухали" за 4 часа (79% в статистике). Сузил
            # до ×1.2, ближе к реально наблюдаемому размаху движения.
            tp1 = _round_price(price + actual_risk * 1.2)
            tp2 = _round_price(price + actual_risk * 2.5)
        ...
        if len(tp_candidates) >= 3:
            ...
            # 🟢 Симметрично LONG-ветке выше: risk×1.5 -> risk×1.2 по тем же
            # данным (средний MFE +0.60% против требуемых минимум ~1.5%).
            tp1 = _round_price(price - actual_risk * 1.2)
            tp2 = _round_price(price - actual_risk * 2.5)
            tp3 = _round_price(price - actual_risk * 4.0)
...
CANDLES_PER_TRADING_DAY_15M = 35  # ~8.75ч основной сессии MOEX / 15 мин - приблизительно, для нормировки
...
def calculate_volume_percentile(df_closed: pd.DataFrame, df_daily: pd.DataFrame = None) -> int:
    ...
    """
    try:
        # 🟢 isinstance вместо "is not None" - если asyncio.gather(...,
        # return_exceptions=True) вернул Exception вместо DataFrame,
        # len(Exception) упал бы TypeError'ом (уже был бы пойман try/except
        # ниже и не крашил бота, но лучше явно не путать ошибку с "нет данных").
        if isinstance(df_daily, pd.DataFrame) and len(df_daily) >= 15:
            # 🟢 РАНЬШЕ: cur_vol = объём ОДНОЙ 15-мин свечи × 4, сравнивался
            # с hist_vols = объём ЗА ВЕСЬ ДЕНЬ (~35 таких свечей). Час
            # (грубо) против дня целиком - структурно разные по масштабу
            # величины, поэтому объём почти всегда "не проходил" (0-20-й
            # перцентиль) даже в совершенно нормальные торговые дни - не
            # потому что объём был реально низким, а потому что сравнение
            # было некорректным само по себе. Теперь сравниваем ОДНУ свечу
            # с ТИПИЧНЫМ объёмом одной свечи в предыдущие дни (дневной
            # объём / кол-во свечей в сессии) - сопоставимые величины.
            hist_vols_per_candle = df_daily["volume"].dropna().values / CANDLES_PER_TRADING_DAY_15M
            cur_vol = float(df_closed["volume"].iloc[-1])
            percentile = (hist_vols_per_candle < cur_vol).mean() * 100
...
# 🟢 Раньше фьючерсы вообще не распознавались этой функцией и молча падали
# в DEFAULT_TIER3_PROFILE (самый мягкий порог по тренду среди всех профилей,
# 55) - независимо от того, Si это или нишевый контракт. Даём фьючерсам
# свой профиль, ближе к TIER2 у акций, а не автоматически самый слабый.
DEFAULT_FUTURES_PROFILE = {"tier": "fut", "min_trend": 60, "min_vol_percentile": 65, "news_weight": 1.0}
...
def get_instrument_profile(ticker: str) -> dict:
    ...
    if tk in INSTRUMENT_PROFILES:
        ...
        return DEFAULT_FUTURES_PROFILE
...
def compute_tech_score(df: pd.DataFrame, mode_cfg: dict,
    ...
    # 🟢 Раньше mode_cfg принимался параметром, но НИГДЕ в этой функции не
    # использовался - LOW/MID/HARD не менял ни один порог, только подпись
    # в заголовке. Теперь режим сдвигает пороги гейтов относительно
    # базового профиля тикера (tier), с разумными границами, чтобы HARD не
    # пропускал вообще всё, а LOW не блокировал вообще всё.
    mode_cfg = mode_cfg or {}
    min_trend = profile["min_trend"] + mode_cfg.get("trend_adj", 0)
    min_trend = max(30, min(90, min_trend))

    min_vol_p = max(35, profile["min_vol_percentile"] - (15 if is_morning else 0))
    min_vol_p = min_vol_p + mode_cfg.get("vol_percentile_adj", 0)
    min_vol_p = max(20, min(90, min_vol_p))

    exec_min = 50 + mode_cfg.get("exec_min_adj", 0)
    exec_min = max(20, min(80, exec_min))

    gate_trend_pass  = trend_score >= min_trend
    gate_volume_pass = vol_percentile >= min_vol_p
    gate_exec_pass   = exec_score >= exec_min

    gates_summary = {
        "tier": profile["tier"], "direction": direction,
        "trend_score": trend_score, "gate_trend": gate_trend_pass, "min_trend": min_trend,
        "vol_percentile": vol_percentile, "gate_vol": gate_volume_pass, "min_vol_p": min_vol_p,
        "exec_score": exec_score, "gate_exec": gate_exec_pass, "exec_min": exec_min,
    }
...
async def check_index_participation_alignment(ticker: str, tech_signal: str) -> tuple[bool, str]:
    ...
    try:
        # Динамика РЕАЛЬНОГО индекса IMOEX (не акции биржи MOEX) за ~1 час
        df_moex = await fetch_imoex_index_candles(days=2, interval=10)
        if df_moex is None or len(df_moex) < 6:
            return False, ""

        close_moex_now  = float(df_moex["close"].iloc[-1])
        close_moex_past = float(df_moex["close"].iloc[-6])
...
async def analyze_stock(ticker: str, tf: str = DEFAULT_TF, mode_cfg: dict = None, run_ai: bool = True) -> dict | None:
    if mode_cfg is None:
        mode_cfg = TRADE_MODES[get_scan_mode()]
    ticker = ticker.upper()
    if ticker not in MOEX_STOCKS:
    ...
    try:
        ...
        # 🟢 Раньше сбор новостей (RSS по компании/сектору/сырью/макро,
        # 4 конкурентных сетевых запроса каждый по ~20 источникам) выполнялся
        # БЕЗУСЛОВНО, даже при run_ai=False - хотя единственный потребитель
        # news_items дальше по коду находится ВНУТРИ if run_ai: (сам вызов
        # ai_evaluate_news). То есть "быстрый чисто технический" проход
        # _run_scan (используется автосканером на ВСЁМ вотчлисте каждые
        # 10 минут) на деле тянул полный новостной фон по каждому тикеру и
        # тут же его выбрасывал - именно то, что run_ai=False должен был
        # исключить по своему собственному комментарию в коде. Новости
        # нужны только когда мы реально зовём ИИ, поэтому убираем их из
        # быстрого прохода - для тикеров, ставших сигналом, их всё равно
        # дотянет _evaluate_and_edit_signal (там уже есть свой fallback
        # "если news_items пустые - дособрать").
        if run_ai:
            ...
            # 🟢 Подмешиваем ещё активные факты по компании (дивиденды/байбек/
            # допэмиссия/санкции), которые уже выпали из короткого окна свежести
            # RSS, но по своему типу событий должны ещё учитываться (см.
            # EVENT_DURATION_DAYS). Без этого допэмиссия недели назад просто
            # исчезала бы из анализа через 10-120 минут как обычная новость.
            try:
                sustained_facts = _load_ticker_facts(ticker)
                if sustained_facts:
                    news_items = news_items + sustained_facts
            except Exception as sf_err:
                logger.debug(f"_load_ticker_facts {ticker}: {sf_err}")
        else:
            imoex_res, htf_res = await asyncio.gather(imoex_task, htf_task, return_exceptions=True)
    ...
    # 🟢 Считаем календарь/дивиденды ДО вызова ИИ (раньше считались после -
    # и в промпт вообще не попадали, ИИ никогда не видел ни ЦБ, ни отсечек).
    ...
    if run_ai:
        try:
            news_ai = await ai_evaluate_news(
                ...
                cal_check=cal_check, div_check=div_check,
            )
        except Exception:
    ...
    result = {
        ...
        "mtf_trends": mtf_trends, "daily_poc": daily_poc, "gates_summary": gates_summary,
}

    # 🟢 /export_log читает RK_SIGNAL_LOG, но ничего и никогда в этот ключ не
    # писало - команда всегда отвечала "лог пуст", хотя её собственное
    # сообщение обещало "записи появляются автоматически при каждом /analyze
    # и при работе сканера". Пишем сюда при каждом успешном analyze_stock -
    # и при ручном /analyze, и при каждом тикере автоскана.
    try:
        _rpush_log(RK_SIGNAL_LOG, {
            "ts": datetime.now(timezone.utc).isoformat(timespec="seconds"),
            "ticker": ticker, "tf": tf, "price": price,
            ...
            "macd_div": macd_div, "htf_trend": htf_trend, "imoex_regime": imoex_regime.get("regime", "") if isinstance(imoex_regime, dict) else "",
            "candle_progress_pct": round(candle_progress_pct, 1),
            "daily_atr_progress_pct": round(daily_atr_progress_pct, 1),
        }, max_len=5000)
    except Exception as log_err:
        logger.debug(f"RK_SIGNAL_LOG append {ticker}: {log_err}")
...
async def update_figi_data() -> tuple:
    ...
    try:
        ...
        def _index_instrument(ticker: str, inst: dict, default_currency: str) -> None:
            """Индексирует инструмент по тикеру, не давая валютному инструменту
            (доллар/др.) затирать уже найденный рублёвый с ТЕМ ЖЕ тикером -
            короткие тикеры типа "T" совпадают у разных бумаг на разных
            биржах (Т-Банк на MOEX и AT&T на NYSE оба "T"), и раньше
            последний обработанный инструмент с совпавшим тикером просто
            перезатирал предыдущий, независимо от того, тот ли это рынок."""
            currency = inst.get("currency", default_currency).lower()
            existing = api_index.get(ticker)
            if existing and existing.get("currency", "rub").lower() == "rub" and currency != "rub":
                logger.info(
                    f"update_figi_data: тикер '{ticker}' (currency={currency}) пропущен - "
                    f"уже есть рублёвый инструмент с тем же тикером"
                )
                return
            api_index[ticker] = {
                "figi": inst["figi"], "name": inst.get("name", ""),
                "uid": inst.get("uid", ""), "currency": currency,
            }
                ...
                if r1.status == 200:
                    ...
                    for inst in shares_data.get("instruments", []):
                        ...
                        if ticker and inst.get("figi"):
                            _index_instrument(ticker, inst, "rub")
            ...
            async with session.post(
                ...
                if r2.status == 200:
                    ...
                    for inst in dr_data.get("instruments", []):
                        ticker = inst.get("ticker", "")
                        if ticker and inst.get("figi"):
                            _index_instrument(ticker, inst, "usd")
        ...
        TICKER_ALIASES = {
            ...
            "YDEX":  ["YDEX", "YDEXRU"],
            "TCSG":  ["TCSG", "TCS", "T"],
            "T":     ["TCSG", "TCS", "T"],  # TCSG/TCS однозначны, "T" совпадает с AT&T на NYSE - проверяем последним
            "X5":    ["X5", "FIVE"],
        ...
        for ticker in list(MOEX_STOCKS.keys()):
            ...
            if found:
                ...
                try:
                    fallback_connector = aiohttp.TCPConnector(ssl=_TRUSTED_SSL_CTX)
                    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10), connector=fallback_connector) as fallback_session:
        ...
        fut_updated = 0
        try:
            fut_connector = aiohttp.TCPConnector(ssl=_TRUSTED_SSL_CTX)
            async with aiohttp.ClientSession(timeout=timeout, connector=fut_connector) as session:
                async with session.post(
                    f"{TINKOFF_API}/tinkoff.public.invest.api.contract.v1.InstrumentsService/Futures",
                    headers=headers, json={"instrumentStatus": "INSTRUMENT_STATUS_BASE"},
                ) as r3:
...
@admin_only
async def cmd_futures_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /futures_toggle — включить/выключить автоскан фьючерсов
    (broadcast сигналов), не трогая сам список отслеживаемых контрактов."""
    new_state = not get_futures_scan_enabled()
    set_futures_scan_enabled(new_state)
    icon = "🟢" if new_state else "🔴"
    status = "включён" if new_state else "выключен"
    await update.message.reply_text(
        f"{icon} <b>Автоскан фьючерсов {status}</b>\n"
        f"{'Бот снова будет присылать сигналы по фьючерсам из /futures_list.' if new_state else 'Список отслеживаемых фьючерсов не тронут - просто сигналы по ним не рассылаются. Включить обратно: /futures_toggle'}",
        parse_mode="HTML",
    )
...
async def _set_bot_commands(app):
    ...
    commands = [
        ...
        BotCommand("why", "🔍 Почему бот принял такое решение по тикеру"),
        ...
        BotCommand("futures_toggle", "📉 Вкл/выкл автоскан фьючерсов"),
    ]
    try:
...
async def cmd_analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args
    if not args:
        await update.message.reply_text("Формат: /analyze TICKER [таймфрейм] [режим: low/mid/hard]")
        return
    ticker = args[0].upper()
    ...
    # 🟢 Раньше третий аргумент (режим) не читался вообще - "/analyze SBER
    # 15m hard" молча анализировал на том режиме, что стоит в /mode, а не
    # на hard. Разовое переопределение только для этого запроса, сохранённую
    # настройку /mode не трогаем.
    mode_override = args[2].lower() if len(args) > 2 else None
    mode_key = mode_override if mode_override in TRADE_MODES else get_user_state(update.effective_chat.id)["mode"]

    msg = await update.message.reply_text(
        f"⏳ Рассчитываю индикаторы для <b>{ticker}</b> [{tf}, {TRADE_MODES[mode_key]['label']}]...", parse_mode="HTML")
...
async def analyze_futures(code: str, tf: str = "15m", mode_cfg: dict = None) -> dict:
    ...
    if not info:
        ...
        if matching:
            ...
            code = matching[0]  # 🟢 иначе df_c.attrs["ticker"] ниже получит нераспознанный сокращённый код
        else:
    ...
    df_c = df.iloc[:-1].copy()
    # 🟢 Раньше здесь не проставлялся df_c.attrs["ticker"] - compute_tech_score
    # внутри вызывает get_instrument_profile(df.attrs.get("ticker", "")), и
    # для ВСЕХ фьючерсов без этой метки ticker был пустой строкой -> все
    # контракты без разбора попадали в DEFAULT_TIER3_PROFILE (самый мягкий
    # порог по тренду: 55 против 65 у голубых фишек), что и объясняло явный
    # перекос "гора сигналов по фьючам, ноль по акциям".
    df_c.attrs["ticker"] = code
    ...
    tech_signal, tech_score, tech_reasons, _gates = compute_tech_score(
...
async def cmd_close_trade(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target == "ALL":
        ...
        for tid in open_ids:
            ...
            await close_trade_safe(tid, "manual", price)
    ...
    closed = await close_trade_safe(trade_id, "manual", price)
    pnl = closed.get("pnl_pct", 0) if closed else 0
    pnl_e = "📈" if pnl >= 0 else "📉"
...
async def cmd_why(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /why TICKER - показывает Decision Log последнего сигнала по бумаге:
    какой гейт что решил и почему. Именно то самое "почему я открыл эту
    сделку", структурированно, а не как текст в саммари.
    """
    if not context.args:
        await update.message.reply_text("Использование: /why TICKER\nНапример: /why SBER")
        return
    ticker = normalize_ticker(context.args[0])
    saved = _load_last_decision(ticker)
    if not saved:
        await update.message.reply_text(
            f"Пока нет сохранённого решения по {esc(ticker)} - сначала должен пройти хотя бы один анализ (скан или /analyze)."
        )
        return

    name = MOEX_STOCKS.get(ticker, ("", ticker, ""))[1]
    lines = [f"🔍 <b>Почему {esc(ticker)} ({esc(name)}) получил такой статус</b>",
             f"<i>{esc(saved.get('ts', ''))}</i>", ""]
    for gate in saved.get("decision_log", []):
        icon = {"PASS": "✅", "CONFIRMED": "✅", "WEAK": "🟡",
                "BLOCK": "🚫", "BLOCKED": "🚫", "WARN": "⚠️",
                "NEWS_ONLY": "📢", "NO_SIGNAL": "⚪", "PENDING": "⏳"}.get(gate.get("result", ""), "⚪")
        lines.append(f"{icon} <b>{esc(gate.get('gate', ''))}</b>: {esc(str(gate.get('result', '')))}")
        if gate.get("detail"):
            lines.append(f"   <i>{esc(str(gate['detail']))}</i>")
    lines.append("")
    lines.append(f"<b>Итоговый статус:</b> {esc(saved.get('filter_status', ''))}")
    await update.message.reply_text("\n".join(lines), parse_mode="HTML")
...
async def cmd_outcome_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🟢 Разбивка по AI confidence - отвечает на вопрос "уверенность ИИ
    # реально предсказывает исход, или это просто число для галочки".
    entries_with_conf = [e for e in entries if e.get("ai_confidence")]
    if entries_with_conf:
        buckets = {"70-79%": [], "80-89%": [], "90-100%": []}
        for e in entries_with_conf:
            c = e.get("ai_confidence", 0)
            if c >= 90:
                buckets["90-100%"].append(e)
            elif c >= 80:
                buckets["80-89%"].append(e)
            elif c >= 70:
                buckets["70-79%"].append(e)
        lines.append("")
        lines.append("🤖 <b>По уверенности AI</b> (ниже 70% вес и так зануляется):")
        for label, items in buckets.items():
            if not items:
                continue
            b_tp = len([i for i in items if "tp" in i.get("outcome", "")])
            b_avg = sum(i.get("pnl_pct", 0) for i in items) / len(items)
            lines.append(f"  {label}: {len(items)} сигн., винрейт {b_tp/len(items)*100:.0f}%, avg {b_avg:+.2f}%")

    # 🟢 Сравнение движков v1 (боевые гейты) vs v2 (SHADOW, экспериментальные
    # баллы) - именно то, ради чего v2 вообще ведётся: не гадать, какая
    # система лучше, а увидеть на реальных цифрах через недели/месяцы.
    by_engine: dict[str, list] = {}
    for e in entries:
        by_engine.setdefault(e.get("engine", "v1"), []).append(e)
    if "v2" in by_engine:
        lines.append("")
        lines.append("⚔️ <b>v1 (боевой) vs v2 (SHADOW):</b>")
        for eng in ("v1", "v2"):
            items = by_engine.get(eng, [])
            if not items:
                continue
            e_tp = len([i for i in items if "tp" in i.get("outcome", "")])
            e_avg = sum(i.get("pnl_pct", 0) for i in items) / len(items)
            label = "v1 боевой" if eng == "v1" else "v2 shadow"
            lines.append(f"  {label}: {len(items)} сигн., винрейт {e_tp/len(items)*100:.0f}%, avg {e_avg:+.2f}%")
        ...
        for attempt in range(3):
            try:
                ...
                if "429" in err_str or "Too Many Requests" in err_str:
                    ...
                    return None
        return None
TRIGGER_LOOKBACK_5M = 8

async def check_5m_trigger(ticker: str, direction: str) -> dict:
    """
    15m = SETUP (есть ли вообще подходящая ситуация - тренд/объём/качество
    входа, это уже считает compute_tech_score). 5m = TRIGGER (есть ли ТОЧКА
    входа прямо сейчас, а не "сетап был 8 минут назад, часть движения уже
    прошла"). Раньше сигнал уходил в эфир сразу по закрытию 15m свечи -
    вплоть до 15 минут между реальным моментом входа и отправкой сигнала.

    Триггер срабатывает при любом из:
    - пробой локального экстремума последних свечей в сторону сигнала
    - всплеск объёма (>=1.3x среднего) со свечой, закрывшейся в сторону сигнала
    """
    try:
        figi = MOEX_STOCKS.get(ticker, FUTURES.get(ticker, ("",)))[0]
        if not figi:
            return {"triggered": True, "reason": "нет FIGI - пропускаем проверку триггера"}

        df = await fetch_candles_tinkoff(figi, "CANDLE_INTERVAL_5_MIN", TRIGGER_LOOKBACK_5M)
        if df is None or len(df) < 4:
            return {"triggered": True, "reason": "мало данных 5m - пропускаем проверку триггера"}

        last = df.iloc[-1]
        prior = df.iloc[:-1]
        last_close, last_open, last_vol = float(last["close"]), float(last["open"]), float(last["volume"])
        avg_vol = float(prior["volume"].mean()) if len(prior) else 0
        vol_spike = avg_vol > 0 and last_vol >= avg_vol * 1.3

        if direction == "long":
            breakout = last_close > float(prior["high"].max())
            momentum_confirm = vol_spike and last_close > last_open
            if breakout:
                return {"triggered": True, "reason": f"пробой локального максимума 5m ({last_close:.2f})"}
            if momentum_confirm:
                return {"triggered": True, "reason": f"объём x{last_vol/avg_vol:.1f} на зелёной свече 5m"}
        else:
            breakdown = last_close < float(prior["low"].min())
            momentum_confirm = vol_spike and last_close < last_open
            if breakdown:
                return {"triggered": True, "reason": f"пробой локального минимума 5m ({last_close:.2f})"}
            if momentum_confirm:
                return {"triggered": True, "reason": f"объём x{last_vol/avg_vol:.1f} на красной свече 5m"}

        return {"triggered": False, "reason": "сетап есть (15m), но точки входа на 5m пока нет"}
    except Exception as e:
        logger.debug(f"check_5m_trigger {ticker}: {e}")
        return {"triggered": True, "reason": f"ошибка проверки триггера ({e}) - пропускаем"}

async def compute_setup_score_v2(ticker: str, tf: str = "15m") -> dict | None:
    """
    SHADOW v2 - альтернативная точечная система скоринга (Trend/Regime/HTF/
    VWAP/Structure/Volume/Volatility = 100 баллов) вместо AND-гейтов
    основного движка (compute_tech_score). Полностью независима: свой расчёт,
    свои пороги, не трогает и не читает состояние боевого движка. Нужна
    только для сравнения через /outcome_stats (поле engine="v2") - реальные
    сделки по ней не открываются, кнопки "войти" нет.
    """
    try:
        ...
        vwap  = float(last.get("vwap", 0) or price)
        atr   = float(last.get("atr", 0) or 0)

        long_score, short_score = 0.0, 0.0

        # Trend (20) - выравнивание EMA
        if price > ema9 > ema20 > ema50 > 0:
            long_score += 20
        elif price < ema9 < ema20 < ema50 and ema50 > 0:
            short_score += 20
        ...
        # Regime (15) - режим IMOEX
        try:
            imoex = await fetch_imoex_regime()
            if imoex.get("regime") == "bull":
                long_score += 15
            elif imoex.get("regime") == "bear":
                short_score += 15
            else:
        ...
        # HTF (15) - тренд на старшем ТФ
        try:
            htf = await get_htf_trend(ticker, figi, tf)
            if htf.get("trend") == "bull":
                long_score += 15
            elif htf.get("trend") == "bear":
                short_score += 15
        except Exception:
            pass

        # VWAP (15) - положение цены относительно VWAP
        if vwap > 0:
        ...
        # Structure (15) - расстояние до ближайшего уровня в сторону сигнала
        try:
            supports, resistances = find_support_resistance(df_closed, price, tf)
            if resistances:
                dist_res = (min(resistances) - price) / price * 100
                if dist_res > 0.3:
                    long_score += 15
            if supports:
                dist_sup = (price - max(supports)) / price * 100
                if dist_sup > 0.3:
                    short_score += 15
        except Exception:
            pass

        # Volume (10) - перцентиль объёма
        try:
            df_daily = await fetch_candles_tinkoff(figi, "CANDLE_INTERVAL_DAY", 30)
            vol_pct = calculate_volume_percentile(df_closed, df_daily)
            if vol_pct >= 60:
                long_score += 10; short_score += 10
        except Exception:
            pass

        # Volatility (10) - ATR в разумных пределах (не мёртвый рынок, не хаос)
        ...
        score = round(max(long_score, short_score), 1)

        return {"ticker": ticker, "direction": direction, "score": score,
                "price": price, "atr": atr, "figi": figi}
    except Exception as e:
        logger.debug(f"compute_setup_score_v2 {ticker}: {e}")
        return None

SHADOW_V2_SCORE_THRESHOLD = 70
_last_shadow_v2_signals: set = set()

async def run_shadow_scanner_v2(app):
    """
    Второй, полностью независимый автоскан ТОЛЬКО по акциям (без фьючерсов,
    без памп/дамп детектора) на альтернативной системе скоринга v2. Пишет в
    тот же чат, явно помеченный как SHADOW - чтобы через недели/месяцы можно
    было сравнить через /outcome_stats, какой движок (v1 - гейты, v2 - баллы)
    реально даёт лучший винрейт/P&L, а не гадать. Реальных сделок не открывает.
    """
    global _last_shadow_v2_signals
    wl = load_watchlist()
    if not wl:
        return

    results = await asyncio.gather(*[compute_setup_score_v2(t) for t in wl], return_exceptions=True)
    candidates = []
    for res in results:
        if isinstance(res, Exception) or res is None:
            continue
        if res["score"] >= SHADOW_V2_SCORE_THRESHOLD:
            candidates.append(res)

    new_candidates = [c for c in candidates if f"{c['ticker']}_{c['direction']}" not in _last_shadow_v2_signals]
    if not new_candidates:
        return

    for c in new_candidates[:5]:
        _last_shadow_v2_signals.add(f"{c['ticker']}_{c['direction']}")
        if len(_last_shadow_v2_signals) > 300:
            _last_shadow_v2_signals = set(list(_last_shadow_v2_signals)[-150:])

        ticker, direction, score, price, atr = c["ticker"], c["direction"], c["score"], c["price"], c["atr"]
        signal_label = "🟩 LONG" if direction == "long" else "🟥 SHORT/ВЫХОД"
        try:
            fresh_candles = await fetch_candles_tinkoff(c["figi"], TF_MAP["15m"][0], TF_MAP["15m"][2])
            supports, resistances = find_support_resistance(fresh_candles, price, "15m")
        except Exception:
            supports, resistances = [], []
        sl_tp = calculate_sl_tp_stocks(signal_label, price, atr, supports, resistances)
        if not sl_tp:
            continue

        name = MOEX_STOCKS.get(ticker, ("", ticker, ""))[1]
        text = (
            f"🧪 <b>SHADOW v2 (эксперимент, НЕ сигнал к входу)</b>\n"
            f"{esc(ticker)} ({esc(name)}) - {'🟢 LONG' if direction=='long' else '🔴 SHORT'}\n"
            f"Score v2: {score}/100 (порог {SHADOW_V2_SCORE_THRESHOLD})\n"
            f"Цена: {price:,.2f} ₽\n"
            f"SL: {sl_tp.get('sl', 0):,.2f} | TP1: {sl_tp.get('tp1', 0):,.2f} | "
            f"TP2: {sl_tp.get('tp2', 0):,.2f} | TP3: {sl_tp.get('tp3', 0):,.2f}\n\n"
            f"<i>Альтернативная система скоринга, только для сравнения с основным "
            f"движком через /outcome_stats. Кнопки входа нет намеренно.</i>"
        )
        for chat_id in SCANNER_CHAT_IDS:
            try:
                await app.bot.send_message(chat_id, text, parse_mode="HTML")
            except Exception as e:
                logger.warning(f"Shadow v2 broadcast failed for {chat_id}: {e}")

        signal_id = f"shadowv2_{ticker}_{int(time.time())}"
        _pending_outcomes_add(signal_id, {
            "ticker": ticker, "tf": "15m",
            ...
            "tech_score": score, "filter_status": "SHADOW_V2",
            "event_type": "", "figi": c["figi"],
            "decision_log": [], "ai_confidence": 0, "engine": "v2",
        })
...
async def _run_scan(tickers: list[str], tf: str, mode_cfg: dict,
    ...
    # 🟢 SETUP (15m) + TRIGGER (5m): 15m-гейты выше уже определили, что
    # ситуация в принципе подходящая (сетап). Но закрытие 15m-свечи -
    # не то же самое, что точка входа прямо сейчас: между двумя сканами
    # (каждые 10 минут) сетап мог быть готов уже 5-14 минут назад, и часть
    # движения уже прошла мимо. Проверяем на 5m, есть ли реальный триггер
    # входа (пробой локального экстремума или объёмный импульс) именно
    # сейчас. Применяется только когда сканируем на 15m (сам сетап) - для
    # старших ТФ (1h/4h) эта логика не подходит, там своя механика входа.
    if tf == "15m" and (long_sigs or short_sigs):
        candidates = [(r, "long") for r in long_sigs] + [(r, "short") for r in short_sigs]
        trigger_results = await asyncio.gather(
            *[check_5m_trigger(r["ticker"], d) for r, d in candidates],
            return_exceptions=True
        )
        confirmed_long, confirmed_short = [], []
        for (r, d), tr in zip(candidates, trigger_results):
            if isinstance(tr, Exception):
                tr = {"triggered": True, "reason": f"ошибка проверки: {tr}"}
            r["trigger_check"] = tr
            if tr["triggered"]:
                (confirmed_long if d == "long" else confirmed_short).append(r)
            else:
                # Сетап подтверждён (прошёл все гейты 15m), но точки входа
                # ещё нет - не выбрасываем совсем, а переводим в watch_sigs,
                # это честнее чем полная тишина: "почти сигнал" видно в /scan.
                r["setup_ready"] = True
                watch_sigs.append(r)
        long_sigs, short_sigs = confirmed_long, confirmed_short
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not long_sigs and not short_sigs:
        # 🟢 Раньше при "не найдено" не было видно ни какой режим реально
        # применился, ни насколько рынок был близко к сигналу - непонятно,
        # это реально тихий рынок или скан не туда смотрит. Теперь
        # показываем лучший "почти сигнал" из watch_sigs (score>=52, но не
        # прошёл все гейты) - если он есть и близко к порогу, значит скан
        # работает штатно, просто рынок пока не дотягивает.
        extra = ""
        if watch_sigs:
            # Сетапы, готовые ждать только триггер 5m, интереснее чем те,
            # что вообще не прошли гейты 15m - показываем их в приоритете.
            setup_ready = [s for s in watch_sigs if s.get("setup_ready")]
            best = max(setup_ready, key=lambda x: x["tech_score"]) if setup_ready \
                else max(watch_sigs, key=lambda x: x["tech_score"])
            best_dir = best.get("gates_summary", {}).get("direction", "")
            best_dir_label = "🟢 LONG" if best_dir == "long" else ("🔴 SHORT" if best_dir == "short" else "")
            if best.get("setup_ready"):
                trig = best.get("trigger_check", {})
                extra = (f"\n\n⏳ Сетап готов, жду точку входа: <b>{esc(best['ticker'])}</b> "
                         f"({best_dir_label}, score {best['tech_score']}) - все гейты 15m пройдены, "
                         f"но на 5m ещё нет триггера ({esc(trig.get('reason', ''))})")
            else:
                extra = (f"\n\nБлиже всего к сигналу: <b>{esc(best['ticker'])}</b> "
                         f"({best_dir_label}, score {best['tech_score']}) - не прошёл все гейты.")
        await msg.edit_text(
            f"😶 <b>Интрадей-сигналов на таймфрейме {tf} не обнаружено.</b>\n"
            f"Режим: {esc(mode_cfg['label'])}{extra}\n<i>{ts}</i>",
            parse_mode="HTML")
        return
...
async def cmd_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Режим сдвигает пороги 3 гейтов (тренд/объём/качество входа) относительно "
        "базовых значений для эшелона бумаги:\n"
        "🟢 LOW - пороги строже (+10..+15), сигналов меньше, но чище\n"
        "🟡 MID - базовые пороги эшелона без изменений\n"
        "🔴 HARD - пороги мягче (-10..-15), сигналов больше, риск шума выше\n\n"
        "<i>Применяется и к автоскану (сигналы в реальном времени), и к /analyze.</i>",
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("mode_"):
        ...
        # 🟢 Раньше run_scanner_broadcast/run_futures_broadcast были жёстко
        # прописаны на TRADE_MODES["mid"] - выбор режима здесь влиял только
        # на ручной /analyze, а на реальный поток автосигналов вообще не
        # влиял. Теперь /mode двигает и автоскан.
        set_scan_mode(mode)
        await query.edit_message_text(
            f"✅ Режим изменен на: <b>{TRADE_MODES[mode]['label']}</b>\n"
            f"<i>Применяется и к автоскану (сигналы в реальном времени), и к /analyze.</i>",
            parse_mode="HTML")
        ...
        try:
            trade_id = await open_trade_safe(
                ticker, direction, entry, sl, tp1, tp2, tp3, chat_id, tf,
                tech_score=tech_score, filter_status=filter_status,
                anomaly=anomaly, ai_summary=ai_summary,
            )
...
async def _evaluate_and_edit_signal(app, sent_msg, s: dict, chat_id: int, tf: str) -> None:
    ...
    try:
        ...
        news_ai = await ai_evaluate_news(
            ...
            cal_check=cal_info, div_check=div_info,
        )

        filter_status = news_ai.get("filter_status", "CONFIRMED")
        event_type    = news_ai.get("event_type", "")

        # 🟢 DECISION LOG: структурированная запись "почему этот сигнал такой,
        # какой он есть" - каждый гейт со своим результатом, а не одна общая
        # строка filter_status. Хранится вместе с сигналом и переживает до
        # разбора исхода в /outcome_stats и /why.
        min_score_required = get_bot_settings()["min_tech_score_confirmed"]
        decision_log = [
            {"gate": "Технический score", "result": "PASS" if tech_score >= min_score_required else "WEAK",
             "detail": f"{tech_score} (порог {min_score_required})"},
            {"gate": "Календарь/ЦБ", "result": "BLOCK" if cal_info.get("block") else ("WARN" if cal_info.get("warning") else "PASS"),
             "detail": cal_info.get("warning") or "нет значимых событий"},
            {"gate": "Дивиденды/отсечка", "result": "BLOCK" if div_info.get("block") else ("WARN" if div_info.get("dividend_info") else "PASS"),
             "detail": div_info.get("dividend_info") or "нет отсечки рядом"},
            {"gate": "AI новостной фон", "result": filter_status,
             "detail": f"confidence={news_ai.get('confidence', 0)}%, weight={news_ai.get('event_weight', 0):+d}, horizon={news_ai.get('horizon', '?')} - {news_ai.get('summary', '')}".strip()},
        ]
        _save_last_decision(ticker, decision_log, filter_status)
        ...
        if has_direction and sl_tp:
            ...
            _pending_outcomes_add(signal_id, {
                ...
                "decision_log": decision_log, "ai_confidence": news_ai.get("confidence", 0),
...
async def run_scanner_broadcast(app):
    ...
    if not wl:
        return
    long_sigs, short_sigs, watch_sigs, scan_stats = await _run_scan(wl, tf, mode_cfg, exclude_low_liquidity=True)
    ...
    for s in new_sigs[:5]:
        ...
        # 🟢 Обратная сторона той же кросс-проверки: если по этому тикеру
        # и направлению уже недавно (<40 мин) прилетал новостной триггер
        # (run_news_driven_scan), не дублируем тем же самым техническим
        # сигналом - реестр общий (_signal_generated_at), обе стороны его
        # проверяют перед отправкой.
        if _get_signal_age_minutes(s["ticker"], direction) is not None:
            continue
        ...
        initial_text = format_analysis(s, show_news=False)
        ...
        for chat_id in SCANNER_CHAT_IDS:
            try:
                ...
                # 3. Запускаем фоновый ИИ, который через 2 секунды ОТРЕДАКТИРУЕТ это же сообщение прямо в чате!
    ...
    try:
        ...
        if df is None or len(df) < 4:
            return {"action": "wait", "reason": "нет данных свечей", "price": 0,
                    "move_pct": 0, "pumped": False, "dumped": False, "atr": 0}
        ...
        # 🟢 Грубый ATR-прокси (средний true range за доступные свечи) - нужен
        # для немедленного входа по сильной новости, когда цена ЕЩЁ не
        # подтвердила движение (см. needs_atr_sl_tp в run_news_driven_scan).
        # Не заменяет полноценный ATR(14) из calculate_indicators, но за 30
        # минут ожидания подтверждения по сильной новости можно упустить
        # часть движения - это компромисс между скоростью и точностью стопа.
        ...
        if news_weight > 0:
            if pumped and price < high6 * 0.99 and vol_ratio < 0.7:
                ...
                return {"action": "short", "price": price, "move_pct": move_pct,
                        "pumped": pumped, "dumped": dumped, "atr": atr,
...
async def run_news_driven_scan(app):
    ...
    Логика:
    1. Нашли свежую сильную новость по тикеру из watchlist - либо в RSS
       за последние NEWS_MAX_AGE_MINUTES, либо ещё активный "длящийся факт"
       (дивиденды/байбек/допэмиссия, см. EVENT_DURATION_DAYS) - раньше сюда
       попадали только новости младше 30 минут, и допэмиссия/дивиденды за
       вчера уже никогда бы не всплыли здесь.
    2. Смотрим что сделала цена за последние 6 свечей 5m (~30 мин)
    3. AI + price action решают: лонг, шорт или ждать
    4. Если цена ещё НЕ подтвердила движение - шлём просто оповещение без
       кнопки "войти" (кнопка появляется только если action уже long/short
       и есть реальный тех. SL/TP) - именно то самое "NEWS_ONLY" поведение,
       которое раньше не работало в основном сканере вообще.
    """
    ...
    async def _fetch_one(ticker: str):
        try:
            ...
            try:
                news_items = news_items + _load_ticker_facts(ticker)
            except Exception:
                pass
    ...
    for res in results:
        ...
        # 🟢 Раньше ключ дедупа строился из title, а _load_ticker_facts вшивает
        # в title динамический префикс "[длится N.Nд]", который меняется
        # каждый цикл скана - ключ каждый раз новый, и один и тот же
        # длящийся факт (дивиденды/допэмиссия) спамился повторно. Для
        # sustained_fact используем стабильный event, для обычных RSS-новостей
        # оставляем title (он не меняется между циклами).
        stable_id = best_news.get("event") if best_news.get("source") == "sustained_fact" else best_news["title"][:40]
        sig_key   = f"news_{ticker}_{stable_id}"
    ...
    triggered = triggered[:5]

    # 🟢 Раньше цена/действие по каждому тикеру считались ПОСЛЕДОВАТЕЛЬНО
    # (await в for-цикле) - при нескольких одновременных триггерах сообщение
    # не отправлялось, пока не досчитается последний тикер. Теперь считаем
    # все параллельно - общее время = время одного самого медленного
    # запроса, а не сумма всех.
    pa_results = await asyncio.gather(
        *[_analyze_post_news_action(t["ticker"], MOEX_STOCKS.get(t["ticker"], ("", "", ""))[0],
                                     t["news"].get("weight", 0), t["news"]["title"])
          for t in triggered],
        return_exceptions=True,
    )
    ...
    kb = []
    needs_sl_tp: list[tuple[str, str, float]] = []  # (ticker, direction, price)

    for t, pa in zip(triggered, pa_results):
        if isinstance(pa, Exception) or not pa:
            continue
        ticker  = t["ticker"]
        news    = t["news"]
        ...
        if action in ("long", "short") and price > 0:
            # 🟢 Кросс-проверка с техническим сканером: если по этому тикеру
            # и направлению уже недавно (<40 мин) был сигнал от run_scanner_
            # broadcast, не дублируем тем же самым по новостному триггеру -
            # раньше у новостного и технического сканера были полностью
            # раздельные dedup-наборы, и один и тот же вход мог прийти
            # двумя отдельными сообщениями почти одновременно.
            if _get_signal_age_minutes(ticker, action.upper()) is None:
                needs_sl_tp.append((ticker, action.upper(), price))
                _register_signal_time(ticker, action.upper())
            else:
                lines.append(f"   <i>Уже был сигнал по этому направлению недавно - не дублируем кнопку</i>")
                lines.append("")
        elif action == "wait" and abs(weight) >= 8 and price > 0 and pa.get("atr", 0) > 0:
            # 🟢 Раньше на "ждать" кнопки не было вообще - ждали до 30 минут
            # подтверждения ценой, и если новость реально сильная, часть
            # движения к этому моменту уже съедена. Для действительно
            # сильных новостей (|вес|>=8) даём немедленный вход по грубому
            # ATR вместо support/resistance (их ещё нет без движения цены) -
            # стопы шире обычного, риск явно помечен в тексте кнопки и
            # сообщения. Для слабых/средних новостей (<8) кнопки по-прежнему
            # нет - ждём подтверждения, как и раньше.
            atr = pa["atr"]
            direction = "LONG" if weight > 0 else "SHORT"

            if _get_signal_age_minutes(ticker, direction) is not None:
                lines.append(f"   <i>Уже был сигнал по этому направлению недавно - не дублируем кнопку</i>")
                lines.append("")
                continue
            _register_signal_time(ticker, direction)

            if direction == "LONG":
                sl, tp1, tp2, tp3 = price - atr * 1.5, price + atr * 2, price + atr * 3, price + atr * 4
            else:
                sl, tp1, tp2, tp3 = price + atr * 1.5, price - atr * 2, price - atr * 3, price - atr * 4
            ...
            _pending_trades[key] = {"ticker": ticker, "direction": direction,
                                     "entry": price, "sl": sl, "tp1": tp1, "tp2": tp2, "tp3": tp3,
                                     "ts": time.time()}
            action_emoji = "🟢 ЛОНГ" if direction == "LONG" else "🔴 ШОРТ"
            kb.append([InlineKeyboardButton(
                f"⚠️ {action_emoji} {ticker} РАНО (ATR, без подтверждения)",
                callback_data=f"enter2_{key}"
            )])
            lines.append(f"   ⚠️ <i>Ранний вход по ATR доступен - стопы шире обычного, цена ЕЩЁ не подтвердила движение</i>")
            lines.append("")

    if needs_sl_tp:
        quick_results = await asyncio.gather(
            *[analyze_stock(tk, "5m", TRADE_MODES["mid"]) for tk, _, _ in needs_sl_tp],
            return_exceptions=True,
        )
        for (ticker, direction, price), quick in zip(needs_sl_tp, quick_results):
            try:
                if isinstance(quick, Exception) or not quick or "error" in quick or not quick.get("sl_tp"):
                    continue
                sl_tp = quick["sl_tp"]
...
async def run_futures_broadcast(app):
    ...
    if not get_futures_scan_enabled():
        return
    tf       = "15m"
    mode_cfg = TRADE_MODES[get_scan_mode()]
    fwl      = load_futures_watchlist()
...
async def scanner_loop(app):
    ...
    while True:
        try:
            _update_heartbeat("scanner_loop")
            ...
            if is_trading_day and hour == 7 and minute < 2:
                ...
                _pd_sent.clear()
                # 🟢 _cache/_news_cache - обычные dict без лимита размера и
                # без TTL-очистки (в отличие от _AI_CACHE, у которого лимит
                # 500 записей). При активном использовании /analyze с разными
                # тикерами/интервалами они могут расти без ограничения. Чистим
                # раз в день на утреннем ресете - дешёво и не мешает
                # внутридневному кэшированию.
                _cache.clear()
                _news_cache.clear()
            ...
            if is_trading_time and SCANNER_CHAT_IDS:
                ...
                try:
                    await asyncio.gather(
                        ...
                        _safe_scan("shadow_v2", run_shadow_scanner_v2(app)),
                    )
...
async def post_init(app):
    ...
    asyncio.create_task(run_weekend_briefing_loop(app))
...
def main():
    ...
    application.add_handler(CommandHandler("why",             cmd_why))
    ...
    application.add_handler(CommandHandler("futures_toggle", cmd_futures_toggle))
