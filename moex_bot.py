...
FUTURES = {
    ...
    "EuM6":  ("FUTEU0926000", "Евро/Рубль (Eu)",      "валюта",   1.0,    1000),
    "RiU6":  ("FUTRI0926000", "Индекс РТС (Ri)",      "индекс",   10.0,   1),
    "MXM6":  ("FUTMX0926000", "Индекс MOEX (MIX)",    "индекс",   0.25,   1),
    "BRU6":  ("FUTBR0926000", "Нефть Brent (BR)",     "товар",    0.01,   10),
    "GDU6":  ("FUTGD0926000", "Золото (Gold)",         "товар",    0.1,    1),
    "NGM6":  ("FUTNG0926000", "Природный газ (NG)",    "товар",    0.001,  1),
    "SRM6":  ("FUTSR0926000", "Серебро (Silver)",      "товар",    0.01,   1),
    "SBU6":  ("FUTSB0926000", "Сбербанк (SBER)",      "акция",    1.0,    100),
    "GZU6":  ("FUTGZ0926000", "Газпром (GAZR)",        "акция",    1.0,    100),
    "LKM6":  ("FUTLK0926000", "Лукойл (LKOH)",         "акция",    1.0,    10),
    "GKM6":  ("FUTGK0926000", "Норникель (GMKN)",      "акция",    1.0,    10),
    "RNM6":  ("FUTRN0926000", "Роснефть (ROSN)",       "акция",    1.0,    100),
    "NKM6":  ("FUTNK0926000", "Новатэк (NVTK)",        "акция",    1.0,    10),
    "VBM6":  ("FUTVB0926000", "ВТБ (VTBR)",            "акция",    0.5,    10000),
    "MNM6":  ("FUTMN0926000", "МТС (MTSS)",            "акция",    1.0,    100),
    "AFM6":  ("FUTAF0926000", "Аэрофлот (AFLT)",       "акция",    1.0,    1000),
}

_instrument_currency: dict[str, str] = {}  

FUTURES_USD_PRICED = {"BRU6", "NGM6", "GDU6", "SRM6"}  
...
async def fetch_candles_moex(ticker: str, interval: str, limit: int) -> pd.DataFrame | None:
    ...
    url = (
        f"https://iss.moex.com/iss/engines/stock/markets/shares/boards/TQBR/securities/"
        f"{ticker}/candles.json?interval={moex_interval}"
    ...
    for row in raw:
        try:
            rows.append({
                "timestamp": pd.Timestamp(row[columns.index("begin")]),
...
# === SECTOR_KEYS для быстрого определения сектора по тексту новости ===
SECTOR_KEYS = {
    ...
    "финансы":     {"финансы", "фондовый", "индекс", "moex", "rts", "биржа", "ibm", "ipo"},
    "энергетика":  {"электроэнергия", "уголь", "энергоноситель", "iea", "генерация", "мощность"},
    "АПК":         {"агро", "зерно", "удобрение", "сельхоз", "пшеница", "урожай"},
    "IT":          {"it", "айти", "технологии", "software", "цифровой", "искусственный интеллект", "ai"},
...
def tag_sectors_by_text(text: str) -> list[str]:
    """Определяет сектора по ключевым словам в тексте новости."""
    text_low = text.lower()
    matched = []
