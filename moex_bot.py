...
# Для обратной совместимости с проверками `if gemini_model:`
...
MOEX_STOCKS = {
    ...
    "TCSG":  ("BBG00QPYJ5H0", "Т-Банк (TCS)",          "банки",
              {"т-банк", "тинькофф", "tcs", "tinkoff bank"}),
    ...
    "FIVE":  ("BBG00JXPFBN0", "X5 Group",              "ритейл",
              {"x5 group", "x5 retail", "пятёрочка", "перекрёсток", "икс5"}),
...
# ══════════════════════════════════════════════
FUTURES = {
    "SiM6":  ("FUTSI0626000", "Доллар/Рубль (Si)",    "валюта",   1.0,    1000),  # RUB
...
# Объявлен здесь, так как используется ниже для commodity-фьючерсов
_instrument_currency: dict[str, str] = {}  # figi -> "rub" | "usd" | "eur"

# Товарные фьючерсы цены которых в USD — регистрируем для конвертации
FUTURES_USD_PRICED = {"BRM6", "NGM6", "GDM6", "SRM6"}  # нефть, газ, золото, серебро
...
# ══════════════════════════════════════════════
# ПРАЗДНИКИ MOEX — биржа не работает
# ══════════════════════════════════════════════
MOEX_HOLIDAYS_2026 = {
    "2026-01-01", "2026-01-02", "2026-01-05", "2026-01-06", "2026-01-07",
    "2026-01-08", "2026-01-09",  # Новый год / Рождество
    "2026-02-23",                 # День защитника Отечества
    "2026-03-09",                 # Женский день (перенос)
    "2026-05-01", "2026-05-04",   # Праздник Весны и Труда
...
def is_trading_day_moex() -> bool:
    ...
    # MOEX_HOLIDAYS_2026 содержит даты только для 2026 года. Если год не
    # совпадает, "date_str not in MOEX_HOLIDAYS_2026" молча даёт True для
    # ЛЮБОЙ даты — включая настоящие праздники — потому что ни одна дата
    # другого года физически не может встретиться в множестве, где все
    # ключи начинаются с "2026-". Явно логируем это раз за процесс, чтобы
    # проблема не терялась молча после смены года.
    if now_msk.year != 2026:
        ...
        # Минимальный fallback — новогодние каникулы MOEX исторически всегда
        # покрывают первые дни января, это не заменяет полный список
        # праздников, но хотя бы не даёт сигналы в самый очевидный нерабочий период.
...
def _get_futures_suffix() -> str:
    ...
    # Экспирации: март(H), июнь(M), сентябрь(U), декабрь(Z)
    # Переходим на следующий за 15 числа месяца экспирации
    month = now.month
    day   = now.day
    if month <= 3 and not (month == 3 and day > 15):
        return f"H{year2}"   # март
    elif month <= 6 and not (month == 6 and day > 15):
        return f"M{year2}"   # июнь
    elif month <= 9 and not (month == 9 and day > 15):
        return f"U{year2}"   # сентябрь
    elif month <= 12 and not (month == 12 and day > 15):
        return f"Z{year2}"   # декабрь
    else:
        # После 15 декабря — следующий год март
...
# Группы коррелирующих фьючерсов
# Если несколько из одной группы дали один сигнал — показываем только лучший

# Маппинг фьючерс → базовый актив (для HTF-тренда)
FUTURES_BASE_ASSET = {
    "SBM6": "SBER",  "GZM6": "GAZP",  "LKM6": "LKOH",
    "GKM6": "GMKN",  "RNM6": "ROSN",  "NKM6": "NVTK",
    "AFM6": "AFLT",  "MNM6": "MTSS",  "VBM6": "VTBR",
}
FUTURES_CORR_GROUPS = [
    {"MXM6", "RIM6"},                          # индексы MOEX
    {"SiM6", "EuM6"},                          # валюты
    {"BRM6", "NGM6"},                          # нефть/газ
    {"GDM6", "SRM6"},                          # металлы
    {"SBM6", "GZM6", "LKM6", "GKM6",
     "RNM6", "NKM6", "AFM6", "MNM6", "VBM6"}, # акции
]
...
def filter_correlated_futures(sigs: list) -> tuple[list, list]:
    ...
    best    = []
    also    = []
    # Сортируем по скору — лучший первый
    sorted_sigs = sorted(sigs, key=lambda x: -x["tech_score"])
    for s in sorted_sigs:
        ticker = s["ticker"]
        # Найдём группу этого тикера
        group = next((g for g in FUTURES_CORR_GROUPS if ticker in g), None)
        if group is None:
            best.append(s)
            continue
        # Уже есть представитель этой группы с тем же направлением?
...
_cache: dict = {}
_news_cache: dict = {}
_pending_trades: dict = {}  # {key: {data..., "ts": timestamp}} — TTL 30 мин
...
# ══════════════════════════════════════════════
# RETRO SIGNAL OUTCOMES — отдельный от журнала сделок трекер
# ══════════════════════════════════════════════
# Хранит исход КАЖДОГО сигнала с CONFIRMED/WEAK (не только тех что взяли в
# реальную сделку). Через 2 часа после сигнала бот проверяет: цена дошла до
# TP1, до SL, или сигнал протух без движения. Это даёт полную статистику
# по эффективности сигналов, а не только по тем что трейдер решил взять руками.
RK_SIGNAL_OUTCOMES = "moex:signal_outcomes"  # hash: signal_id -> json
...
RK_OUTCOME_STATS = "moex:outcome_stats"  # список финализированных исходов, для /outcome_stats
...
RK_SECTOR_MODIFIERS = "moex:sector_modifiers"  # hash: sector -> json modifier state

# Все реальные сектора, встречающиеся в MOEX_STOCKS — используется системой
# геополитических/событийных модификаторов ниже, чтобы работать со ВСЕМИ
# секторами (банки, ритейл, IT...), а не только с commodity-секторами
# (нефтегаз, металлы, золото), для которых уже был отдельный механизм.
ALL_SECTORS = {
...
# ══════════════════════════════════════════════
# НАСТРОЙКИ БОТА — регулируемые пороги без правки кода
# ══════════════════════════════════════════════
# Дефолты — используются если настройка не менялась через /settings
BOT_SETTINGS_DEFAULTS: dict[str, float] = {
    "fact_weight_ai_trigger":    6,    # |fact_weight| выше — вызываем AI даже без ценовой аномалии
    "news_max_age_min":          120,  # новость старше — не участвует в скоринге (2-й эшелон)
    "news_max_age_liquid_min":   10,   # то же для ликвидных тикеров (SBER, GAZP...)
    "min_tech_score_confirmed":  61,   # ниже — сигнал не считается CONFIRMED
    "sl_min_risk_pct":           1.0,  # минимальный риск SL в % от цены входа
    "sl_max_risk_pct":           3.0,  # максимальный риск SL в % от цены входа
    "time_exit_loss_threshold":  -1.5, # % — при таком минусе и низком прогрессе закрываем по таймауту
    "time_exit_tp1_progress":    30,   # % прогресса к TP1 ниже которого таймаут закрывает сделку
    "pump_dump_vol_ratio":       3.0,  # множитель объёма для детектора пампа/дампа
    "news_dedup_similarity":     80,   # символов заголовка для дедупликации новостей
}
...
class RedisChatDataPersistence(BasePersistence):
    ...
    # ── Методы добавленные в новых версиях PTB — обязательны к реализации ──
    async def refresh_bot_data(self, bot_data: dict) -> None:
        pass  # bot_data не персистим

    async def refresh_chat_data(self, chat_id: int, chat_data: dict) -> None:
        # Подгружаем сохранённые данные чата в текущий chat_data объект
        saved = self._load_one(chat_id)
        if saved:
            chat_data.update(saved)

    async def refresh_user_data(self, user_id: int, user_data: dict) -> None:
        pass  # user_data не персистим

    async def get_conversations(self, name: str):
        return {}

    async def update_conversation(self, name: str, key: tuple, new_state: object) -> None:
        pass  # conversation handlers не используются в боте
...
async def _check_trade(trade_id: str, t: dict, price: float, trades_cache: dict = None) -> str | None:
    ...
    # Логика: таймаут НЕ закрывает сделку принудительно если:
    # - SL не выбит (риск ограничен)
    # - Цена не ушла глубже -1.5% от входа
    # - Это не 2-й день удержания подряд
    # Закрываем только если прогресс к TP1 < 30% И потери > 1.5% (сигнал не работает)
    if status == "open":
        try:
            ...
            if elapsed_min >= max_hold:
                ...
                # Сколько дней удерживается
                days_held = int(elapsed_min // (24 * 60))

                # Условия принудительного закрытия по таймауту:
                # 1. Сделка уже 2+ дня — закрываем без вопросов
                # 2. Глубокий минус И нет прогресса к TP1 — сигнал не работает
                force_close = False
                close_reason_text = ""
                ...
                if force_close:
                    ...
                    # Таймаут истёк, но сделка живёт дальше — уведомляем
                    if abs(elapsed_min - max_hold) < 5:  # только один раз ~в момент истечения
...
async def _force_close_all(app, fetch_price_fn, moex_stocks):
    ...
    for trade_id, t in open_trades.items():
        ...
        # Фьючерсы всегда закрываем на EOD
        # Акции в минусе без б/у SL — закрываем
        # Акции в плюсе или с б/у SL — переносим
        ...
        try:
            if should_close:
                ...
                # Переносим на следующий день — только уведомление
...
async def monitor_trades_loop(app, fetch_price_fn):
    ...
    while True:
        try:
            ...
            # Проверяем закрытие сессии — 23:48 МСК (только один раз)
            now_msk = datetime.now(timezone.utc).astimezone(timezone(timedelta(hours=3)))
            if now_msk.hour == 23 and now_msk.minute >= 48:
                today_str = now_msk.strftime("%Y-%m-%d")
                # Раньше защита от повтора хранилась в атрибуте функции
                # (monitor_trades_loop._last_eod_date) — это память процесса,
                # которая обнуляется при любом рестарте контейнера (деплой,
                # OOM, health-check). Если рестарт происходил несколько раз
                # подряд внутри 12-минутного окна 23:48-23:59, EOD выполнялся
                # заново при каждом старте — отсюда несколько одинаковых
                # уведомлений "перенос на завтра" подряд.
                # SETNX в Redis атомарен и переживает рестарт: первый процесс,
                # который успел его выставить, "забирает" право выполнить EOD
                # на сегодня; все последующие (включая параллельные при
                # двойном деплое) увидят что ключ уже занят и не станут
                # повторять рассылку.
                r = _get_redis()
                ...
                if r:
                    try:
                        ...
                        # Без Redis — fallback на старую защиту в памяти процесса
...
# ══════════════════════════════════════════════
DEFAULT_WATCHLIST = [
    "SBER", "GAZP", "LKOH", "GMKN", "ROSN", "NVTK", "YNDX", "TATN",
    "CHMF", "NLMK", "MOEX", "VTBR", "MGNT", "FIVE", "AFLT",
    # Расширенный список — 2-й эшелон, чтобы не сканировался раз в месяц
...
def load_watchlist() -> list[str]:
    ...
    # data может прийти не списком (None, строка, число) — из битой записи
    # в Redis/файле. Не пытаемся итерировать что попало.
    if not isinstance(data, list):
        if data:  # не пустое, но и не список — это реально мусор, логируем
...
# Курс USD/RUB — заполняется при первом запросе, кэш 5 минут
# (_instrument_currency объявлен выше, рядом с FUTURES_USD_PRICED)
...
async def _get_usd_rub() -> float:
    ...
    try:
        ...
        pass
    return _usd_rub_rate if _usd_rub_rate > 0 else 75.0  # fallback (актуализирован на июнь 2026, курс ~73-77)
...
async def fetch_stock_data(ticker: str, tf: str = DEFAULT_TF):
    ...
    # Sanity-check: изредка Tinkoff API отдаёт глитч-свечу с ценой на порядки
    # ниже нормы (наблюдалось: CHMF ~595₽ → одна свеча 0.046₽). Отсекаем
    # свечи где close отличается от медианы последних 20 более чем в 5 раз —
    # это не реальное движение рынка, а битые данные.
...
def check_dividend_cutoff(ticker: str) -> dict:
    ...
    # Берём ближайшее по времени событие (может быть в прошлом или будущем)
    ev, ev_time, days_diff = min(relevant, key=lambda x: abs(x[2]))

    # Уже прошла (более 12 часов назад) — гэп уже случился, это не блок
    if days_diff < -0.5:
        return {
    ...
    # Предстоящая отсечка в пределах 3 торговых дней (≈4.5 календарных с выходными)
    if days_diff <= 4.5:
...
async def auto_update_figi_loop():
    ...
    while True:
        try:
            ...
            if count:
                logger.info(f"Auto FIGI update: {count} tickers")
            if err:
...
# Ранжирование надёжности источников. Tier 1 — официальные/проверенные
# информагентства. Tier 2 — крупные деловые издания. Tier 3 — агрегаторы
# и неофициальные Telegram-каналы (быстрые, но менее проверенные).
# Используется как доп. контекст при показе новости, не блокирует вес.
RSS_SOURCE_TIER = {
    "www.moex.com":           1,
    "www.interfax.ru":        1,
    "tass.ru":                1,
...
# Telegram-каналы через RSSHub получают tier 3 по умолчанию (см. ниже,
# RSSHUB_URL домен присваивается динамически — обрабатывается через .get() с tier=2)
...
RUSSIAN_NEWS_RSS = [
    ...
    _tg("interfaxonline"),   # Самый быстрый оперативный новостной поток
    _tg("cbr_official"),     # ЦБ РФ официальный
    _tg("markettwits"),      # Короткие новости рынка
    _tg("rosbiznessman"),    # Деловые новости РБК
    _tg("moexpert"),         # Новости МосБиржи
    _tg("smartlabonline"),   # Корпоративные новости
    _tg("russianmacro"),     # Макроэкономические показатели РФ
    _tg("selfinvestor"),     # Разбор отчетностей компаний
]

# ══════════════════════════════════════════════
# COMMODITY NEWS — мировые сырьевые рынки
# ══════════════════════════════════════════════
# Только надёжные источники с реальным XML RSS (проверены)
COMMODITY_NEWS_RSS = [
    # OilPrice.com — нефть, газ, энергетика (стабильный публичный RSS)
    "https://oilprice.com/rss/main",
    # Mining.com — металлы, горнодобыча
    "https://www.mining.com/feed/",
    # Kitco — золото, серебро, металлы (стабильный публичный RSS)
    "https://www.kitco.com/rss/KitcoNews.xml",
    # MarketWatch — общий рыночный поток (содержит commodities)
    "https://www.marketwatch.com/rss/topstories",
    # Yahoo Finance Commodities — нефть/золото/серебро/медь фьючерсы
    "https://finance.yahoo.com/rss/2.0/headline?s=CL=F,GC=F,SI=F,HG=F",
]

# Ключевые слова для фильтрации по секторам (английский)
COMMODITY_SECTOR_MAP: dict[str, list[str]] = {
    "нефтегаз": [
        ...
        # Геополитика — события которые двигают нефть, но не содержат "oil"
...
# Секторы для которых commodity-новости актуальны
...
# показываются для ЛЮБОГО тикера — ставка ЦБ двигает весь рынок, не только банки.
MACRO_KEYWORDS = [
    # ЦБ РФ и ставка
...
# Возраст новости для ЛИКВИДНЫХ тикеров — цена реагирует быстрее,
# новость старше 10 минут на SBER/GAZP уже полностью в цене
NEWS_MAX_AGE_LIQUID_MINUTES = 10
NEWS_CACHE_TTL       = 90    # секунд — обновляем каждые 1.5 минуты

# Стоп-слова для блокировки военных/географических новостей
# Такие заголовки не должны маппиться на компании через общие слова типа "санкции", "удар"
...
# Не-финансовый шум который агрегаторы (RSSHub Telegram-каналы) иногда
# подмешивают в общий поток — спорт, шоу-бизнес, культура, погода.
# Такие заголовки не должны попадать в финансовый анализ ни для одной компании.
OFFTOPIC_STOPWORDS = {
...
def classify_news_item(title: str) -> dict:
    ...
    for pattern, event_label, weight, is_corp in FACT_PATTERNS:
        # Короткие латинские паттерны (<=4 символа, только буквы/цифры) требуют
        # word-boundary — иначе "spo" матчит "Sportaran", "ecb" матчит "specb" и т.п.
        # Русские и длинные паттерны остаются как substring — коллизий там не бывает.
...
async def fetch_russian_news(ticker: str = "", sector: str = "") -> list[dict]:
    ...
    # Белый список уникальных маркеров — главный критерий специфичности
    if company_markers:
        specific_words.update(company_markers)
    elif company_name:
        # Fallback если маркеров нет — слова длиннее 4 букв без пустышек
        GENERIC_NAME_PARTS = {
    ...
    for url, result in zip(RUSSIAN_NEWS_RSS, results):
        ...
        for item in result:
            ...
            # Блокируем военные/географические новости — они не про компании
            title_lower = title.lower()
            if any(sw in title_lower for sw in MILITARY_GEO_STOPWORDS):
                continue

            # Блокируем не-финансовый шум (спорт, шоу-бизнес) из агрегаторов
            ...
            # ── СТРОГИЙ КРИТЕРИЙ СПЕЦИФИЧНОСТИ ──
            # Новость специфична ТОЛЬКО если тикер упомянут явно
            # или один из уникальных маркеров компании есть в заголовке
            is_specific = (
                _word_match(ticker, title) or
                any(_marker_match(m, title) for m in company_markers)
            )

            # ── ИСПРАВЛЕНИЕ ОШИБОЧНОГО МАППИНГА ──
            # Если в заголовке упоминается другая компания из MOEX_STOCKS —
            # и нашей при этом нет — сбрасываем привязку
            ...
            # Если только generic-слова совпали и новость не специфична — пропускаем
    ...
    # Дедупликация: сначала по смысловому событию (event), затем по тексту.
    # "Газпром снизил дивиденды" из 4 источников с разной формулировкой —
    # у всех одинаковый event ("снижение дивидендов"), поэтому ловится сразу,
    # а не только при совпадении первых N символов заголовка (настройка).
...
async def fetch_commodity_news(sector: str) -> list[dict]:
    ...
    for url, result in zip(COMMODITY_NEWS_RSS, results):
        ...
        for item in result:
            ...
            # Берём только свежие (не старше NEWS_MAX_AGE_MINUTES)
...
async def fetch_sector_news(sector: str) -> list[dict]:
    ...
    # Макро ищем и в русских источниках (ставка ЦБ, рубль), и в мировых (Fed, геополитика)
    ...
    for url, result in zip(all_sources, results):
        ...
        for item in result:
            ...
            title_lower = title.lower()
            if any(sw in title_lower for sw in MILITARY_GEO_STOPWORDS):
                # Военные новости пропускаем ВЕЗДЕ, кроме явной макро-эскалации
    ...
    unique.sort(key=lambda x: x["age_min"])
    unique = unique[:3]  # макро-новостей не должно быть много — только самое важное
...
# ══════════════════════════════════════════════
# СЕКТОРАЛЬНЫЕ ГЕОПОЛИТИЧЕСКИЕ/СОБЫТИЙНЫЕ МОДИФИКАТОРЫ
# ══════════════════════════════════════════════
# Проблема которую решает этот блок: технический анализ (VWAP/RSI/объём)
# работает одинаково независимо от того, что в мире произошло только что
# сильное геополитическое событие, долгосрочно двигающее целый сектор в
# одну сторону (пример: США ударили по Ирану — нефть должна расти, вместе
# с ней весь нефтегазовый сектор, но если 15-минутный технический паттерн
# ещё показывает откат — бот слал SHORT, полностью игнорируя контекст).
#
# Решение: AI классифицирует каждую сильную геополитическую/макро новость —
# какие сектора затронуты, в какую сторону, насколько сильно и на сколько
# дней это разумно учитывать. Результат хранится в Redis с истечением
# срока. analyze_stock проверяет активный модификатор для сектора тикера
# и если сигнал идёт ПРОТИВ направления модификатора — понижает
# CONFIRMED/WEAK статус и добавляет понятное русское предупреждение.

SECTOR_MODIFIER_DEFAULT_DAYS = 3  # геополитика живёт днями, не часами (в
                                  # отличие от NEWS_MAX_AGE_MINUTES=120 для
                                  # обычных корпоративных фактов)
...
def _load_sector_modifiers() -> dict[str, dict]:
    ...
    # Чистим протухшие записи, чтобы hash не рос бесконечно
    if expired_keys:
        try:
...
def _set_sector_modifier(sector: str, direction: str, strength: int,
    ...
    try:
        mod = {
            ...
            "reason":      reason,     # короткое описание для UI на русском
            "set_at":      time.time(),
...
async def classify_geopolitical_impact(title: str, source_context: str = "") -> dict | None:
    """
    AI-классификация геополитической/макро новости: какие сектора
    затронуты, в какую сторону, насколько сильно, на сколько дней
    разумно это учитывать. Возвращает None если новость не достаточно
    значима для секторального модификатора (обычный шум).

    Вызывается только для новостей уже прошедших keyword-фильтр
    commodity/macro (COMMODITY_SECTOR_MAP / MACRO_KEYWORDS) — не на
    каждую новость подряд, это было бы дорого и шумно.
    """
    if not gemini_model and not groq_client:
        return None

    sectors_list = ", ".join(sorted(ALL_SECTORS))
    prompt = f"""Ты аналитик российского фондового рынка (MOEX). Оцени новость на предмет
долгосрочного влияния на сектора рынка.

НОВОСТЬ: "{title}"
{f'КОНТЕКСТ: {source_context}' if source_context else ''}

Доступные сектора: {sectors_list}

Если новость — геополитическое событие, макроэкономическое решение, или
крупный рыночный сдвиг, который РАЗУМНО ожидать повлияет на конкретный
сектор(а) в течение нескольких ДНЕЙ (не часов, не шум одного торгового дня) —
определи это влияние. Если новость незначительна, локальна для одной
компании, или это просто рыночный комментарий без реального event — верни
"significant": false.

Примеры значимых событий: военная эскалация в нефтедобывающем регионе
(нефтегаз бычий), санкции против экспорта металлов (металлы медвежий),
резкое повышение ключевой ставки ЦБ (банки/финансы разнонаправленно),
кризис поставок газа в Европу (нефтегаз бычий для России).

Ответь СТРОГО в формате JSON, без пояснений:
{{
  "significant": true/false,
  "sectors": ["сектор1", "сектор2"],
  "direction": "bullish" или "bearish",
  "strength": число от 1 до 10,
  "days_relevant": число дней (1-7) на которое это разумно учитывать,
  "reason_ru": "короткое объяснение на русском, 1 предложение, понятное трейдеру"
}}

Если "significant": false — остальные поля можно не указывать."""
    ...
    try:
        ...
        sectors = [s for s in obj.get("sectors", []) if s in ALL_SECTORS]
        if not sectors:
            return None
...
async def run_geopolitical_scan():
    ...
    while True:
        try:
            ...
            # Дополнительно проверяем общие macro-новости — могут затрагивать
            # несколько секторов сразу (санкции, ставка ЦБ и т.д.)
            try:
...
# ══════════════════════════════════════════════
# AI NEWS EVALUATION (GEMINI)
# ══════════════════════════════════════════════
def _groq_call(prompt: str, model_idx: int = 0) -> str:
    """Groq fallback — используется если Gemini недоступен или вернул пустой ответ."""
    if not groq_client:
        return ""
        ...
        for attempt in range(3):
            try:
                ...
                if "rate_limit" in err.lower() or "429" in err:
                    wait = 2 ** attempt
                    logger.warning(f"Groq rate limit {GROQ_MODELS[i]}, retry in {wait}s")
                    time.sleep(wait)
                else:
...
def _gemini_call(prompt: str) -> str:
    """Gemini primary (google-genai SDK) → Groq fallback при ошибке или пустом ответе."""
    if _gemini_client and GEMINI_MODEL:
        for attempt in range(3):
            try:
                ...
                if result:
                    return result
                logger.warning(f"Gemini attempt {attempt+1}: пустой ответ")
            except Exception as e:
                err = str(e)
                if "429" in err or "quota" in err.lower():
                    logger.warning(f"Gemini quota/rate limit, attempt {attempt+1}")
    ...
    # — Groq fallback —
    logger.info("AI: переключаюсь на Groq fallback")
...
async def ai_evaluate_news(news_items: list[dict], ticker: str, sector: str,
    ...
    # Объединяем e-disclosure и RSS факты
    all_facts = []
    ...
    # AI вызываем если:
    # 1. Есть свежие факты И (ценовая аномалия ИЛИ e-disclosure) — как раньше, ИЛИ
    # 2. Keyword-скорер уже нашёл сильное событие — не нужно дожидаться
    #    аномалии цены, дивиденды/байбек важны сами по себе
    _settings = get_bot_settings()
    ...
    # Диагностика: почему AI не вызвался — критично для отладки эффективности
...
{{"explains_movement": true/false, "weight": число, "summary": "одно предложение"}}"""
    ...
    # Проверка контртренда: сигнал против старшего таймфрейма — понижаем статус.
    # HTF показывает не сиюминутный шум, а структурный тренд — торговать против
    # него можно, но это осознанный повышенный риск, не должно тихо проходить
    # как обычный CONFIRMED.
    ...
    # Слабый технический сигнал не должен получать CONFIRMED даже если
    # новостной вес сильный — новость подтверждает движение, но не заменяет
    # саму техническую картину (VWAP/EMA/RSI/объём).
    min_score_required = get_bot_settings()["min_tech_score_confirmed"]
    is_tech_too_weak = tech_score < min_score_required

    # Логика BLOCKED: только жёсткий негатив блокирует вход
...
async def ai_trade_review(
    ...
    # MTF тренды + HTF + IMOEX режим из meta
...
def get_session_phase() -> dict:
    ...
    # Официальное расписание MOEX (рынок акций), проверено на 2026 год:
    # 06:50-06:59 — аукцион открытия утренней сессии
    # 07:00-09:49 — утренняя доп. сессия (только ликвидные бумаги, повышенная волатильность)
    # 09:50-10:00 — аукцион открытия основной сессии
    # 10:00-18:39 — основная сессия (дневной клиринг ~14:00-14:05, не блокирующий)
    # 18:40-18:50 — аукцион закрытия
    # 19:00-19:04 — аукцион открытия вечерней сессии
...
def find_support_resistance(df: pd.DataFrame, price: float, tf: str = "15m"):
    # Адаптивная глубина и сила пивота по таймфрейму
    ...
    highs: dict[float, int] = {}  # уровень → сила (кол-во подтверждений)
    lows:  dict[float, int] = {}
    ...
    for i in range(n_side, n - n_side):
        ...
        # Пивот-хай: максимум среди n_side соседей с каждой стороны
        if all(h >= float(r.iloc[i + k]["high"]) for k in range(-n_side, n_side + 1) if k != 0):
            # Сила уровня: сколько ещё свечей касалось ±0.3% от него
            touches = sum(
                1 for j in range(n)
    ...
    # Кластеризация близких уровней (±0.5% от цены считаем одним уровнем)
    def _cluster(levels: dict) -> list[float]:
        ...
        for i, lvl in enumerate(sorted_lvls):
            ...
            for g in group:
                skip.add(g)
            # Берём уровень с наибольшей силой в группе
...
def calculate_sl_tp_stocks(signal: str, price: float, atr: float,
    ...
    def _round_price(p):
        # Если задан шаг цены инструмента (фьючерсы FORTS) — округляем к нему.
        # Иначе используем дефолтную точность по величине цены (акции).
    ...
    if is_long:
        ...
        # Защита: SL не должен быть равен или выше цены входа
        ...
        # Защита: SL не должен быть равен или ниже цены входа
        if sl <= price or actual_risk < min_tick:
            sl          = _round_price(price + min_risk)
            actual_risk = sl - price
            risk_pct    = actual_risk / price * 100
...
# ══════════════════════════════════════════════
# ТЕХНИЧЕСКИЙ АНАЛИЗ — POC (POINT OF CONTROL)
# ══════════════════════════════════════════════
def calculate_daily_poc(df_daily) -> float | None:
    ...
    try:
        ...
        for hi, lo, vol in zip(highs, lows, volumes):
            if hi <= lo or vol <= 0:
                continue
            # Распределяем объём свечи пропорционально перекрытию с каждым бином
            candle_range = hi - lo
...
async def analyze_stock(ticker: str, tf: str = DEFAULT_TF, mode_cfg: dict = None) -> dict | None:
    ...
    if df_result is None or len(df_result) < 20:
        # Fallback: пробуем взять больший период (x2)
        try:
            interval, _, limit = TF_MAP.get(tf, TF_MAP[DEFAULT_TF])
    ...
    # ── МУЛЬТИТАЙМФРЕЙМ ТРЕНДЫ И ДНЕВНОЙ POC ──
    ...
    try:
        ...
        if not isinstance(news_res, Exception) and news_res is not None:
            # Для ликвидных тикеров новость старше порога уже полностью
            # в цене — не показываем её как "актуальную" для сигнала
            _settings = get_bot_settings()
    ...
    # ── Новый новостной блок: движение → причина → объяснение ────────────
    # Аномальное движение цены как триггер поиска причины
    price_anomaly = detect_price_anomaly(df_closed, tf)

    # e-disclosure был удалён — реального публичного API не существует
    # (проверено: /API/Search не существует, только платный "Шлюз" с авторизацией).
    # Источник фактов для AI теперь — только RSS (news_items), уже отфильтрованный
    # на релевантность выше в fetch_russian_news.
    edisclosure_items: list = []

    # AI объясняет связь движения с фактами (не просто "оцени новость")
    ...
    # ── Дивидендная отсечка — отдельная жёсткая проверка ────────────────
    # В отличие от check_calendar_block (окно 2 часа, штраф к скору),
    # отсечка известна заранее и должна блокировать LONG за несколько дней —
    # гэп вниз на размер дивиденда гарантирован, это не вероятностный риск.
    ...
    if div_check["block"] and "LONG" in final_signal:
        ...
        # Отсечка была недавно (вчера/сегодня утром) — цена гэпнула вниз
        # МЕХАНИЧЕСКИ (реестр закрылся, это не рыночное давление продавцов),
        # но технический анализ (VWAP/RSI/тренд) видит это падение так же,
        # как обычное медвежье движение и штампует SHORT на артефакте
        # методологии, а не на реальном настроении рынка. Особенно вводит
        # в заблуждение в выходные/низколиквидные периоды, где в стакане
        # почти нет реальных сделок, только технический аукцион.
    ...
    # ── Секторальный геополитический/событийный модификатор ─────────────
    # AI периодически (run_geopolitical_scan) анализирует commodity/macro
    # новости и определяет: затрагивает ли новость конкретный сектор
    # долгосрочно (дни, не часы), в какую сторону и насколько сильно.
    # Если технический сигнал идёт ПРОТИВ этого фактора — понижаем статус
    # с понятным русским предупреждением, не блокируя жёстко (решение
    # всё равно остаётся за трейдером).
    try:
    ...
    try:
        ...
        if candle_range > 0:
            if "LONG" in tech_signal:
                ...
                candle_progress_pct = (price - last_low)  / candle_range * 100
            # Цена может уйти за диапазон прошлой свечи (гэп, сильный импульс) —
            # ограничиваем метрику до 0-100%, иначе получаются бессмысленные
            # значения типа -500% или +200% которые ломают интерпретацию.
    ...
    # ── Дневной ATR progress ──────────────────────────────────────────────
    # Показывает сколько % дневного ATR уже пройдено сегодня.
    # Если цена прошла >80% дневного ATR — день "выдохся", новые входы рискованны.
    # Пример: дневной ATR = 2%, цена уже прошла 1.8% от открытия = 90% ATR → поздно.
    daily_atr_progress_pct  = 0.0
    daily_atr_progress_note = ""
    try:
        # Берём дневные свечи из df_closed (агрегируем интрадей в дни)
    ...
    # ── Логирование сигнала для анализа эффективности AI ────────────────
    try:
        # tech_score имеет смысл только когда есть направление (LONG/SHORT).
        # На NO_SIGNAL строках он был мусорным числом (например 98 без сигнала) —
        # обнуляем явно, чтобы при разборе лога не путать "сильный сигнал
        # без направления" с реальным сигналом.
    ...
    # ── Регистрация сигнала для ретро-проверки исхода ────────────────────
    # Только реальные сигналы с направлением и хотя бы минимальным фильтром —
    # NO_SIGNAL/BLOCKED не проверяем, там нечего проверять.
    try:
        ...
        if has_direction and _fs in ("CONFIRMED", "WEAK", "NEWS_ONLY") and sl_tp:
            ...
            _pending_outcomes_add(signal_id, {
                ...
                # MFE/MAE — максимальное движение цены в пользу и против
                # сделки за время удержания, % от входа. Обновляются на
                # каждом цикле retro_check_signals_loop по мере поступления
                # новых наблюдений цены (раз в 15 минут — не непрерывное
                # покрытие, но честное приближение при текущей частоте опроса).
                "mfe_pct":       0.0,
                "mae_pct":       0.0,
            })
...
def format_analysis(result: dict) -> str:
    ...
    # Показываем ценовую аномалию если есть
    ...
    # AI объяснение связи движения и факта
    ...
    # Предупреждение о дальней ATR цели
...
def _futures_base_code(ticker: str) -> str:
    ...
    if m:
        return m.group(1)
    return ticker[:2].upper()  # fallback на старое поведение если формат неожиданный
...
# Тексты всех кнопок меню — используется чтобы отличить нажатие кнопки
# от ввода произвольного текста (например числа для /settings).
# Строится из самой клавиатуры, чтобы не рассинхронизироваться при правках.
...
async def cmd_analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сохраняем метаданные анализа для передачи в open_trade при нажатии "Войти"
...
async def analyze_futures(code: str, tf: str = "15m", mode_cfg: dict = None) -> dict:
    ...
    # HTF тренд через базовый актив (для акционных фьючерсов)
    htf_trend = {}
...
async def cmd_futures(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "error" not in result:
        ...
        if sl_tp and result.get("tech_signal") in ("🟩 LONG", "🟥 SHORT/ВЫХОД"):
            ...
            t2 = sl_tp.get("tp2", 0)
            t3 = sl_tp.get("tp3", 0)
            # Короткий ключ вместо прямого форматирования цены в callback_data —
            # см. пояснение в /futures_list выше про лимит Telegram 64 байта.
...
async def cmd_scan_futures(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for s in (long_sigs + short_sigs):  # кнопки на ВСЕ сигналы
        ...
        if sl_tp and sl_tp.get("sl"):
            ...
            # Короткий ключ вместо полной callback_data — фьючерсы с 4 знаками
            # после запятой и высокой ценой (валютные пары, товарные контракты)
            # могут превышать лимит Telegram в 64 байта при прямом
            # форматировании cены в callback_data (проверено: до 71 байта
            # для реалистичного курса доллара). Тот же паттерн что уже
            # используется в run_futures_broadcast.
...
def detect_price_anomaly(df: pd.DataFrame, tf: str = "15m") -> dict | None:
    ...
    # Минимальные пороги по TF — на старших TF шум больше
    min_pct = {"5m": 0.6, "15m": 0.8, "1h": 1.2, "4h": 1.8, "1d": 2.5}.get(tf, 0.8)
    min_vol = 2.0   # объём должен быть минимум в 2 раза выше среднего

    recent   = df.tail(3)       # последние 3 свечи
    baseline = df.tail(30).head(27)  # предыдущие 27 свечей для нормы
...
def detect_pump_dump(df: pd.DataFrame, tf: str = "15m") -> dict | None:
    ...
    # Настройка из /settings — глобальный множитель поверх дефолтного vol_mult
    # для каждого TF. Дефолт 3.0 совпадает с базовым vol_mult "5m" — при
    # изменении настройки все ТФ ужесточаются/ослабляются пропорционально.
...
async def cmd_pump_dump(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in results:  # кнопки на ВСЕ найденные памп/дамп сигналы
        # Короткий ключ вместо прямого форматирования цены в callback_data —
        # памп/дамп сканирует и фьючерсы (4 знака после запятой), тот же
        # риск превышения лимита Telegram 64 байта что и в /futures_list.
...
async def cmd_calendar(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сравниваем по естественному ключу (name, datetime_utc), а не по полному
    # равенству словарей — upcoming-элементы содержат добавленное поле
    # hours_ahead, которого нет в all_ev, поэтому "e not in upcoming" через
    # dict equality было всегда True, и "Далее" полностью дублировало
    # "Ближайшие" вместо показа только оставшихся событий.
...
async def cmd_outcome_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Разбивка по filter_status — CONFIRMED должен быть лучше WEAK
    ...
    # MFE/MAE — насколько сильно цена двигалась в пользу/против сделки
    # за время удержания, независимо от финального исхода. Даёт понять,
    # адекватно ли расставлены SL/TP, а не только "сработало или нет".
    ...
    if has_mfe_mae:
        ...
        # Самое полезное: MFE на проигрышных сделках — был ли шанс на
        # прибыль до того как выбило SL. Если MFE на sl_hit заметно
        # положительный — TP слишком далеко или сигнал выхода запаздывает.
        ...
        # MAE на выигрышных сделках — насколько глубоко в минус уходила
        # цена перед тем как дойти до TP. Если глубоко — можно рискнуть
        # сузить SL, не потеряв в винрейте.
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Фильтр по периоду: /stats 7d, /stats 30d, без аргумента — всё время
    period_days = None
    if context.args:
...
async def cmd_trades(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверка на запрос подробного журнала (история)
...
async def cmd_export_log(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # entries идут от новых к старым (LPUSH) — разворачиваем для хронологии
    ...
    csv_bytes = output.getvalue().encode("utf-8-sig")  # BOM для Excel/Google Sheets
...
def _format_scan_row(s: dict) -> str:
    ...
    # Дневной ATR — раньше показывался только в /analyze, в списке сигналов
    # сканера отсутствовал, хотя данные уже вычисляются в analyze_stock.
...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если больше половины тикеров не удалось получить — это не "сигналов нет",
    # а сбой сети/API. Явно сообщаем, а не молчим как при обычном пустом скане.
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("mode_"):
        ...
        # Пересчёт уровней от ТЕКУЩЕЙ цены через полный анализ — не сдвиг
        # старых уровней, а честный новый поиск support/resistance.
        parts = data.split("_")
        try:
            ticker    = parts[1]
            direction = parts[2]
        ...
        # Пользователь явно подтвердил вход по устаревшей цене сигнала —
        # пропускаем проверку актуальности, открываем как есть.
        parts = data.split("_")
        try:
            ticker    = parts[1]
            direction = parts[2]
        ...
        try:
            ticker    = parts[1]
            direction = parts[2]
            entry     = float(parts[3])  # цена НА МОМЕНТ СИГНАЛА, не текущая
            sl        = float(parts[4])
            tp1       = float(parts[5])
            tp2       = float(parts[6])
            tp3       = float(parts[7])
        ...
        # ── Проверка возраста сигнала — независимо от дрейфа цены ──────
        # Цена может почти не измениться (в пределах порога дрейфа), но сам
        # технический сетап (VWAP-пересечение, RSI-уровень, всплеск объёма)
        # актуален только в узком окне после появления сигнала. Бот сам
        # пишет в сообщении "сигналы актуальны ~30 мин" — но раньше это
        # никак не проверялось программно при нажатии кнопки.
        ...
        # Сигнал мог быть сгенерирован несколько минут назад — за это время
        # цена могла уйти далеко от entry. Входить по устаревшей цене
        # означает другой реальный риск и R/R чем показано в сигнале.
        ...
        if real_price and entry:
            ...
            # Направление ухудшения зависит от направления сделки:
            # LONG — хуже если цена уже ВЫШЕ входа (заходим дороже)
            # SHORT — хуже если цена уже НИЖЕ входа (заходим по менее выгодной цене)
            is_worse = ((direction == "LONG" and real_price > entry) or
                       (direction == "SHORT" and real_price < entry))

            if drift_abs >= 1.0:
                # Слишком большое расхождение — просим явно решить, входить или нет
                await query.answer(
                ...
                # ── Почему нет кнопки "пересчитать сдвигом" ──────────────
                # Механический сдвиг (SL/TP += цена_сейчас - цена_сигнала)
                # двигает всю сетку уровней параллельно, но SL/TP должны
                # стоять у РЕАЛЬНЫХ уровней поддержки/сопротивления текущей
                # цены (это то, что calculate_sl_tp_stocks и так уже делает
                # через find_support_resistance) — а не на том же расстоянии
                # от новой цены, что и от старой. Если цена успела дойти,
                # скажем, до старого TP1, простой сдвиг откроет сделку ровно
                # на уровне, где движение уже остановилось, и подвинет TP1
                # ещё дальше по инерции, без всякой технической переоценки.
                # Правильный пересчёт — это свежий /analyze, который заново
                # ищет уровни от текущей цены, а не отъезжает от старых.
                await query.message.reply_text(
        ...
        # AI ревью — параллельно грузим все виды новостей (как в analyze_stock)
        try:
            sector = ""
        ...
        # Редактируем сообщение — добавляем AI блок
        ai_block = (
            f"\n\n🤖 <b>AI ревью:</b>\n{esc(review)}"
            if review else
...
# Время генерации каждого сигнала — ticker_direction -> unix timestamp.
# Позволяет enter_ callback проверить возраст сигнала независимо от того,
# насколько сильно сдвинулась цена: сигнал может быть технически ещё в
# пределах порога дрейфа цены (<1.5%), но сам сетап (VWAP/RSI/объём) уже
# развалился за прошедшее время — цена не главный индикатор устаревания.
_signal_generated_at: dict[str, float] = {}
SIGNAL_STALE_AFTER_MIN = 40  # сигналы "актуальны ~30 мин" — даём 10 мин запаса
...
# ── Состояние для уведомлений о сбоях сканера ────────────────────────────
# consecutive_failures: сколько циклов подряд скан не смог завершиться
# last_failure_notify_ts: когда последний раз слали алерт (троттлинг — не
...
SCANNER_FAILURE_NOTIFY_COOLDOWN = 3600  # не чаще раза в час при повторных сбоях
...
async def run_scanner_broadcast(app):
    ...
    # Показываем время скана и подсказку об актуальности
    lines = [
...
async def _analyze_post_news_action(
    ...
    try:
        ...
        # Текущий объём vs среднее — индикатор продолжения или выдыхания
        ...
        if news_weight > 0:
            if pumped and price < high6 * 0.99 and vol_ratio < 0.7:
                # Памп был, цена откатила, объём падает → продают в новость
                ...
                # Движение продолжается с объёмом → лонг
                ...
                # Цена не двинулась — рынок не верит новости
        ...
        # Негативная новость
            ...
            if dumped and price > low6 * 1.01 and vol_ratio < 0.7:
                # Дамп был, цена отскочила, объём падает → buy the dip
                return {"action": "long", "price": price, "move_pct": move_pct,
                        "pumped": pumped, "dumped": dumped,
...
async def run_news_driven_scan(app):
    ...
    # ── Параллельная загрузка новостей по всем тикерам ──────────────────
    # Раньше это был последовательный for-цикл с await на каждый тикер —
    # если RSS/RSSHub тормозил на паре тикеров, весь скан зависал на часы,
    # блокируя scanner_loop (он ждёт этот await через asyncio.gather выше
    # по стеку) и следующие сигналы уходили с многочасовым опозданием.
    ...
    try:
        # Жёсткий потолок на весь этап загрузки новостей — 60 секунд.
        # Лучше пропустить один цикл сканирования новостей, чем заблокировать
        # scanner_loop на неопределённое время.
    ...
    for t in triggered[:5]:
        ...
        # Анализ price action после новости
        ...
        # Кнопку показываем только если есть конкретное действие
                ...
                if quick and "error" not in quick and quick.get("sl_tp"):
                    ...
                    if sl and t1:
                        kb.append([InlineKeyboardButton(
                            ...
                            callback_data=f"enter_{ticker}_{direction}_{price:.2f}_{sl:.2f}_{t1:.2f}_{t2:.2f}_{t3:.2f}"
                        )])
            except Exception as e:
...
async def run_futures_broadcast(app):
    ...
    # Фильтр корреляции — из каждой группы только лучший сигнал
    ...
    # "Смотри также" — коррелирующие сигналы которые не попали в основной список
    ...
    # Коррелирующие сигналы — упоминаем кратко
    if new_also:
    ...
    for s in new_sigs[:4]:
        ...
        if sl_tp and sl_tp.get("sl"):
            ...
            p  = s["price"]; sl = sl_tp.get("sl", 0)
            t1 = sl_tp.get("tp1", 0); t2 = sl_tp.get("tp2", 0); t3 = sl_tp.get("tp3", 0)
            # Короткий ключ вместо полного callback_data (Telegram лимит 64 байта)
            key = f"{s['ticker']}_{direction}"[:20]
            _cleanup_pending_trades()
            _pending_trades[key] = {"ticker": s["ticker"], "direction": direction,
                                    "entry": p, "sl": sl, "tp1": t1, "tp2": t2, "tp3": t3,
        ...
        try:
            ...
            for signal_id, data in pending.items():
                try:
                    ...
                    cur_price = await fetch_last_price_tinkoff(figi)
                    if not cur_price:
                        continue  # попробуем в следующий цикл
                    ...
                    if not entry or entry <= 0:
                        # Битая запись (entry=0) — не пытаемся делить на ноль каждые
                        # 15 минут до таймаута, чистим сразу.
                        _pending_outcomes_remove(signal_id)
                        continue

                    # ── MFE/MAE: обновляем максимум/минимум движения ────────
                    # Текущее движение цены относительно входа, в % и с учётом
                    # направления (LONG: рост цены — в плюс; SHORT: падение —
                    # в плюс). Сравниваем с уже накопленным максимумом/
                    # минимумом и берём лучший из двух, а не просто текущее
                    # значение — иначе последний замер перед разрешением
                    # сигнала перезаписывал бы любой более сильный экскурс,
                    # случившийся между проверками.
                    ...
                    # Таймаут — 4 часа без исхода считаем протухшим сигналом
                    ...
                    if outcome:
                        ...
                        # Финальная точка (SL/TP/протухание) сама по себе тоже
                        # часть экскурса — учитываем её в итоговых MFE/MAE.
                        ...
                        # Сигнал ещё не разрешился — сохраняем накопленные
                        # MFE/MAE обратно в pending, чтобы прогресс не терялся
                        # к следующему циклу проверки через 15 минут.
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Ожидание числового значения для изменяемой настройки ────────────
    # Проверяем ТОЛЬКО если сообщение не команда и не кнопка меню, и запрос
    # не протух (>5 минут) — иначе человек нажимает "Скрыть меню" пока бот
    # ждёт число, получает "нужно числовое значение", и awaiting_setting
    # зависает навсегда, перехватывая все последующие сообщения.
    awaiting_key = context.chat_data.get("awaiting_setting")
    awaiting_ts  = context.chat_data.get("awaiting_setting_ts", 0)
    is_stale     = (time.time() - awaiting_ts) > 300  # 5 минут
...
async def post_init(app):
    ...
    # Проверка надёжности commodity RSS-источников при старте
    _rss_check_urls = COMMODITY_NEWS_RSS[:3]  # проверяем первые 3 (OilPrice, Mining.com, Kitco)
    session = _get_http_session()
    headers = {"User-Agent": "Mozilla/5.0 (compatible; MOEXBot/1.0)"}
    ...
    # Первое обновление FIGI фьючерсов — ждём завершения перед стартом
    # сканера, чтобы не было гонки состояний (скан фьючерсов на старых,
    # возможно уже экспирированных FIGI, пока обновление ещё не пришло).
    try:
...
def main():
    ...
    application.run_polling(
        ...
        drop_pending_updates=False,  # уже сделано в post_init через delete_webhook
        poll_interval=2.0,
        timeout=30,
    )
