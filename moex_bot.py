...
# ══════════════════════════════════════════════
# REDIS PERSISTENCE — асинхронная реализация
# ══════════════════════════════════════════════
...
class RedisChatDataPersistence(BasePersistence):
    ...
    Все методы объявлены как асинхронные для совместимости с PTB v20+.
    """
    ...
    # --- Асинхронные методы для chat_data ---
    async def get_chat_data(self) -> dict:
        all_data = self._load()
        # Преобразуем строковые ключи в int для корректной работы библиотеки
    ...
    # --- Асинхронные методы-заглушки для user_data ---
    ...
    # --- Асинхронные методы-заглушки для bot_data ---
    ...
    # --- Асинхронные методы-заглушки для conversations ---
    ...
    # --- Асинхронные методы-заглушки для callback_data ---
...
COMMODITY_NEWS_RSS = [
    # Reuters — самый надёжный, стабильный XML
    "https://feeds.reuters.com/reuters/businessNews",
    "https://feeds.reuters.com/reuters/companyNews",
    # FT Commodities (без paywall на RSS заголовки)
    "https://www.ft.com/rss/home/commodities",
    # MarketWatch Markets
    "https://feeds.marketwatch.com/marketwatch/marketpulse/",
    # OilPrice.com — нефть, газ, энергетика
    "https://oilprice.com/rss/main",
    # Mining.com — металлы, горнодобыча
    "https://www.mining.com/feed/",
    # Kitco — золото, серебро, металлы
    "https://www.kitco.com/rss/kitco-news.xml",
    # LME/Metal Bulletin через Fastmarkets
...
def _word_match(word: str, text: str) -> bool:
    """Проверяет вхождение слова с учётом границ — не как substring.
    Решает проблему коротких тикеров типа MOEX матчащих домен moex.com
    в каждой второй новости, или GMK матчащих случайные сочетания букв."""
    if not word:
        return False
...
async def fetch_russian_news(ticker: str = "", sector: str = "") -> list[dict]:
    ...
    for url, result in zip(RUSSIAN_NEWS_RSS, results):
        ...
        for item in result:
            ...
            # ── ИСПРАВЛЕНИЕ ОШИБОЧНОГО ОПРЕДЕЛЕНИЯ НОВОСТИ (ГРУППИРОВКА) ──
            # Исключаем ложный маппинг чужих новостей из-за подвальных тегов в агрегаторах
            other_mentions = False
            for other_tk, (_, other_name, _) in MOEX_STOCKS.items():
                if other_tk == ticker:
                    continue
                # Если в заголовке фигурирует другое название компании или тикер
                if _word_match(other_tk, title) or (other_name.lower() in title.lower() and len(other_name) > 4):
                    # Если целевой тикер при этом в заголовке НЕ упомянут — сбрасываем привязку
                    if not (_word_match(ticker, title) or (company_name.lower() in title.lower() if company_name else False)):
                        other_mentions = True
                        break

            # Особый фильтр для Софтлайн (SOFL) новости которой не должны маппиться на X5 Group (FIVE)
...
def _gemini_call(prompt: str) -> str:
    """Gemini primary → Groq fallback при ошибке или пустом ответе."""
    # — Gemini primary —
    if gemini_model:
        for attempt in range(3):
            try:
...
async def analyze_stock(ticker: str, tf: str = DEFAULT_TF, mode_cfg: dict = None) -> dict | None:
    ...
    try:
        ...
        news_res, commodity_res, imoex_res, htf_res = await asyncio.gather(
            news_task, commodity_task, imoex_task, htf_task,
            return_exceptions=True
        )
    ...
    if data.startswith("mode_"):
        ...
        try:
            ...
            if ticker.upper() in MOEX_STOCKS:
                _, _, sector = MOEX_STOCKS[ticker.upper()]
...
async def run_news_driven_scan(app):
    """
    Новостной сканер — независимый от технического скоринга.

    Идея: если по тикеру из watchlist прошла свежая (<30 мин) релевантная
    новость с сильным весом (|weight| >= 7), это само по себе сигнал для
    внимания трейдера — независимо от того, что в этот момент говорит VWAP.

    В отличие от run_scanner_broadcast (который сначала считает технику,
    потом проверяет новости как фильтр), здесь новость — первична.
    Технический анализ подключается только чтобы подтвердить направление
    и рассчитать SL/TP, если они уместны.
    """
    ...
    for ticker in wl:
        ...
        # Ищем самую свежую и сильную релевантную новость
        strong_fresh = [
            it for it in news_items
            if it.get("is_fact") and not it.get("is_opinion")
            and it.get("is_specific")  # обязательно специфична для компании
            and it.get("age_min", 999) <= 30  # свежая — до 30 минут
            and abs(it.get("weight", 0)) >= 7  # сильный вес
        ]
        if not strong_fresh:
            continue
    ...
    lines = [
        ...
        "<i>Сильная свежая новость — независимо от технической картины</i>",
        "",
    ]
    ...
    for t in triggered[:5]:
        ...
        direction_hint = "возможен LONG" if weight > 0 else "возможен SHORT/выход"

        lines += [
            f"{emoji} <b>{esc(ticker)}</b> — {esc(name)}",
            f"   {esc(news['title'][:120])}",
            f"   Вес: {weight:+d}/10 | {age:.0f} мин назад | {direction_hint}",
            "",
        ]

        # Быстрый технический анализ для SL/TP, не блокирующий показ новости
        try:
            quick = await analyze_stock(ticker, "15m", TRADE_MODES["mid"])
            if quick and "error" not in quick and quick.get("sl_tp"):
                sl_tp = quick["sl_tp"]
                if sl_tp.get("sl"):
                    direction = "LONG" if weight > 0 else "SHORT"
                    p  = quick["price"]; sl = sl_tp.get("sl", 0)
                    t1 = sl_tp.get("tp1", 0); t2 = sl_tp.get("tp2", 0); t3 = sl_tp.get("tp3", 0)
                    kb.append([InlineKeyboardButton(
                        f"✅ Войти {ticker} ({direction}, по новости)",
                        callback_data=f"enter_{ticker}_{direction}_{p:.2f}_{sl:.2f}_{t1:.2f}_{t2:.2f}_{t3:.2f}"
                    )])
        except Exception as e:
            logger.debug(f"quick analysis for news trigger {ticker}: {e}")
...
async def post_init(app):
    ...
    _rss_check_urls = COMMODITY_NEWS_RSS[:3]  # проверяем первые 3 (Reuters, FT, MarketWatch)
    session = _get_http_session()
    headers = {"User-Agent": "Mozilla/5.0 (compatible; MOEXBot/1.0)"}
