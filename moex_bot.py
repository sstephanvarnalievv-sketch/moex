...
# ══════════════════════════════════════════════
# E-DISCLOSURE — поиск корпоративных событий в реальном времени
# ══════════════════════════════════════════════
_edisclosure_cache: dict = {}
EDISCLOSURE_CACHE_TTL = 120  # 2 минуты — быстрее RSS


async def fetch_edisclosure(ticker: str, hours_back: int = 4) -> list[dict]:
    """
    Запрашивает e-disclosure.ru по тикеру за последние hours_back часов.
    Задержка ~1-2 минуты vs 5-15 мин у RSS.
    Возвращает список событий: тип, заголовок, время публикации.
    """
    cache_key = f"ediscl_{ticker}_{hours_back}"
    now = time.time()
    if cache_key in _edisclosure_cache and now - _edisclosure_cache[cache_key]["ts"] < EDISCLOSURE_CACHE_TTL:
        return _edisclosure_cache[cache_key]["items"]

    company_name = MOEX_STOCKS.get(ticker.upper(), ("", ticker, ""))[1]
    # e-disclosure поиск: ищем по названию компании и тикеру
    query = company_name if company_name and len(company_name) > 3 else ticker
    date_from = (datetime.now(timezone.utc) - timedelta(hours=hours_back)).strftime("%Y-%m-%d")

    url = f"https://e-disclosure.ru/API/Search?query={query}&dateFrom={date_from}&pageSize=10"
    items = []
    ...
    try:
        ...
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=8)) as r:
            if r.status != 200:
                _edisclosure_cache[cache_key] = {"items": [], "ts": now}
                return []
            data = await r.json(content_type=None)

            for doc in (data.get("Data") or [])[:8]:
                title    = doc.get("EventFullTitle") or doc.get("EventTitle", "")
                pub_date = doc.get("PublishDate", "")
                doc_type = doc.get("EventTitle", "")
                if not title:
                    continue

                # Парсим время публикации
                age_min = hours_back * 60  # дефолт — считаем старым
                try:
                    from email.utils import parsedate_to_datetime
                    # e-disclosure отдаёт формат "2026-06-13T14:30:00"
                ...
                # Классифицируем событие по типу документа
                cls = classify_news_item(title)
                items.append({
                    "title":      title[:200],
                    "doc_type":   doc_type,
                    ...
                    "is_fact":    True,  # e-disclosure всегда факт, не мнение
                    "is_corporate": True,
                    "is_opinion": False,
                    "is_specific": True,  # всегда специфично для компании
                    "event":      cls["event"] if cls["is_fact"] else doc_type,
                    "weight":     cls["weight"],
                    "effective_weight": cls["weight"] if age_min <= NEWS_MAX_AGE_MINUTES else 0,
                    "link":       doc.get("Url", ""),
                    "matched":    [ticker.lower()],
                })

    except Exception as e:
        logger.debug(f"e-disclosure {ticker}: {e}")

    _edisclosure_cache[cache_key] = {"items": items, "ts": now}
    return items
...
async def analyze_stock(ticker: str, tf: str = DEFAULT_TF, mode_cfg: dict = None) -> dict | None:
    ...
    # Пункт 2: аномальное движение цены как триггер поиска причины
    price_anomaly = detect_price_anomaly(df_closed, tf)

    # Пункт 1: e-disclosure — свежайшие корпоративные факты
    try:
        edisclosure_items = await fetch_edisclosure(ticker, hours_back=4)
    except Exception:
        edisclosure_items = []

    # Пункт 3: AI объясняет связь движения с фактами (не просто "оцени новость")
