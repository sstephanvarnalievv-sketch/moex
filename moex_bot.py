...
_last_watch_broadcast_ts: float = 0.0
_last_watch_ticker_ts: dict[str, float] = {}
WATCH_BROADCAST_COOLDOWN_SEC = 3600  # весь блок WATCH - не чаще раза в час
WATCH_TICKER_COOLDOWN_SEC    = 3600  # один и тот же тикер - не чаще раза в час, даже если score скачет

async def _broadcast_watch_tier(app, watch_sigs: list) -> None:
    """
    WATCH-тир - тикеры со score>=52, которые НЕ прошли все 3 гейта разом
    (тренд/объём/качество входа), но близко. Раньше эти данные считались
    (watch_sigs) и просто выбрасывались - видимости "рынок почти дал сигнал,
    но не хватило объёма" не было вообще, только полная тишина.

    ЭТО НЕ СИГНАЛ: без SL/TP, без кнопки "войти" - чисто "на что смотреть",
    показывается только когда в этом цикле НЕТ ни одного полноценного
    CONFIRMED/WEAK сигнала (чтобы не плодить лишние сообщения поверх
    обычного потока).

    🟢 Раньше дедуп был по f"{ticker}_{score}" - score гуляет на ±1-2 пункта
    между сканами даже без реального движения, поэтому ключ почти каждый
    раз был "новым" и бот спамил одним и тем же тикером каждые 10 минут.
    Теперь: (1) весь блок шлётся не чаще раза в час, (2) один и тот же
    тикер не повторяется чаще раза в час, независимо от точного score.
    """
    global _last_watch_broadcast_ts, _last_watch_ticker_ts
    if not watch_sigs:
        return

    now = time.time()
    if now - _last_watch_broadcast_ts < WATCH_BROADCAST_COOLDOWN_SEC:
        return

    watch_sigs = sorted(watch_sigs, key=lambda x: -x["tech_score"])
    new_watch = [s for s in watch_sigs
                 if now - _last_watch_ticker_ts.get(s["ticker"], 0) >= WATCH_TICKER_COOLDOWN_SEC][:3]
    if not new_watch:
        return

    lines = ["🔍 <b>WATCH - близко к сигналу, но не все гейты пройдены</b>", ""]
    for s in new_watch:
        _last_watch_ticker_ts[s["ticker"]] = now
        if len(_last_watch_ticker_ts) > 300:
            trimmed = sorted(_last_watch_ticker_ts.items(), key=lambda x: -x[1])[:150]
            _last_watch_ticker_ts = dict(trimmed)

        gates = s.get("gates_summary", {})
        direction = gates.get("direction", "")
        dir_label = "🟢 LONG" if direction == "long" else ("🔴 SHORT" if direction == "short" else "⚪")
        failed = []
        if not gates.get("gate_trend"):
            failed.append(f"тренд {gates.get('trend_score', '?')}/{gates.get('min_trend', '?')}")
        if not gates.get("gate_vol"):
            failed.append(f"объём {gates.get('vol_percentile', '?')}/{gates.get('min_vol_p', '?')}")
        if not gates.get("gate_exec"):
            failed.append(f"качество входа {gates.get('exec_score', '?')}/{gates.get('exec_min', '?')}")
        failed_text = "; ".join(failed) if failed else "не хватило одного из гейтов"

        lines.append(f"• <b>{esc(s['ticker'])}</b> ({esc(s['name'])}) - {dir_label}, score {s['tech_score']}")
        lines.append(f"   Не прошло: {esc(failed_text)}")

    lines.append("")
    lines.append("<i>Не торговый сигнал - просто ближайшие кандидаты, если гейты дотянутся на следующем скане.</i>")

    text = "\n".join(lines)
    _last_watch_broadcast_ts = now
    for chat_id in SCANNER_CHAT_IDS:
        try:
            await app.bot.send_message(chat_id, text, parse_mode="HTML")
        except Exception as e:
            logger.warning(f"Watch-tier broadcast failed for {chat_id}: {e}")
...
async def run_scanner_broadcast(app):
    ...
    if not all_sigs:
        await _broadcast_watch_tier(app, watch_sigs)
        return
