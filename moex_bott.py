...
FUTURES_USD_PRICED = {"BRM6", "NGM6", "GDM6", "SRM6"}  
...
async def cmd_diagnostics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rss_count = 0
    for url in COMMODITY_NEWS_RSS:
        try:
            session = _get_http_session()
            # Just test connection, don't fetch full content
            status["✅"].append(f"📰 RSS {url}: источник настроен")
            rss_count += 1
        except:
            pass
    if rss_count > 0:
...
"]
    ...
    report.append(f"
📊 <b>Статистика:</b> всего {total} компонентов, не работает {failed}, предупреждений {warnings}")
