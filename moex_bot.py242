...
MOEX_STOCKS = {
    ...
    "FIVE":  ("BBG00JXPFBN0", "X5 Group",              "ритейл",
              {"x5", "x5 group", "пятёрочка", "перекрёсток", "икс5"}),
...
async def analyze_stock(ticker: str, tf: str = DEFAULT_TF, mode_cfg: dict = None) -> dict | None:
    ...
    # AI оценка новостей с таймаутом - если не успевает, используем чисто тех-сигнал
    try:
        news_ai = await asyncio.wait_for(
            ai_evaluate_news(
        ...
        logger.info(f"analyze_stock {ticker}: AI deferred (timeout 0.5s), will run in background")
        news_ai = {"confirmed": tech_signal, "ai_skip_reason": "deferred", "filter_status": "", "event_type": "", "event_weight": 0, "summary": ""}
    except Exception as ai_err:
        logger.warning(f"analyze_stock {ticker}: AI evaluation error: {ai_err}")
        news_ai = {"confirmed": tech_signal, "ai_skip_reason": "error"}
