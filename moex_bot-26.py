...
class RedisChatDataPersistence(BasePersistence):
    ...
    def _save(self, data: dict) -> None:
        try:
            _rset(RK_CHAT_DATA, json.dumps(data, ensure_ascii=False))
        except Exception as e:
    ...
    for t in triggered[:5]:
        ...
        if action in ("long", "short") and price > 0:
            try:
                ...
                quick = await analyze_stock(ticker, "5m", TRADE_MODES.get("scalp", TRADE_MODES.get("mid")))
