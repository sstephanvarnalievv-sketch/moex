    ...
    if status == "open":
        try:
            ...
            if elapsed_min >= max_hold:
                ...
                # 2. Глубокий минус (>1.5%) И нет прогресса к TP1 — сигнал не работает
                force_close = False
                close_reason_text = ""
                ...
                elif tp1_progress < 0.30 and pnl_pct < -1.5:
                    force_close = True
...
async def ai_evaluate_news(news_items: list[dict], ticker: str, sector: str,
                           ...
                           edisclosure_items: list = None) -> dict:
    """
    ...
    # 1. Есть свежие факты И
    # 2. Есть ценовая аномалия (движение требует объяснения) ИЛИ есть e-disclosure факты
    should_call_ai = (
        gemini_model and all_facts and
        (price_anomaly is not None or bool(edisclosure_items))
    )
...
def calculate_sl_tp_stocks(signal: str, price: float, atr: float,
    ...
    # Минимальный риск — не менее 1% и не менее 2 тиков
    min_tick  = tick_size if tick_size > 0 else (0.01 if price < 1 else 0.001)
    min_risk  = max(price * 0.010, min_tick * 2)  # минимум 1% или 2 тика
...
async def analyze_stock(ticker: str, tf: str = DEFAULT_TF, mode_cfg: dict = None) -> dict | None:
    ...
    try:
        ...
        if not isinstance(news_res, Exception) and news_res is not None:
            # Для ликвидных тикеров новость старше 10 минут уже полностью
            # в цене — не показываем её как "актуальную" для сигнала
            max_age = (NEWS_MAX_AGE_LIQUID_MINUTES if ticker in LIQUID_TICKERS
                       else NEWS_MAX_AGE_MINUTES)
...
async def cmd_export_log(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    fieldnames = [
        ...
        "ai_summary", "macd_div", "htf_trend", "imoex_regime",
