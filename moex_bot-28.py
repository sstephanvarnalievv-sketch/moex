    ...
    while True:
        try:
            ...
            if now_msk.hour == 23 and now_msk.minute >= 48:
                ...
                if getattr(monitor_trades_loop, "_last_eod_date", "") != today_str:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("mode_"):
        ...
        if real_price and entry:
            ...
            if drift_abs >= 1.5:
                # Слишком большое расхождение — просим AI/явно решить, входить или нет
                await query.answer(
                ...
                await query.message.reply_text(
                    ...
                    f"<i>Открой заново /analyze {ticker} чтобы получить актуальные "
                    f"уровни для текущей цены, или используй кнопку ниже чтобы "
                    f"войти по старой цене на свой риск.</i>",
                    parse_mode="HTML",
...
async def run_scanner_broadcast(app):
    ...
    ts_scan = datetime.now().strftime("%H:%M")
