...
async def _broadcast_watch_tier(app, watch_sigs: list) -> None:
    ...
    обычного потока).
    """
    global _last_watch_signals
    if not watch_sigs:
        return

    watch_sigs = sorted(watch_sigs, key=lambda x: -x["tech_score"])
    new_watch = [s for s in watch_sigs if f"{s['ticker']}_{s['tech_score']}" not in _last_watch_signals][:3]
    if not new_watch:
        return

    lines = ["🔍 <b>WATCH - близко к сигналу, но не все гейты пройдены</b>", ""]
    for s in new_watch:
        _last_watch_signals.add(f"{s['ticker']}_{s['tech_score']}")
        if len(_last_watch_signals) > 300:
            _last_watch_signals = set(list(_last_watch_signals)[-150:])
