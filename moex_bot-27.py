...
async def fetch_russian_news(ticker: str = "", sector: str = "") -> list[dict]:
    ...
    # а не только при совпадении первых 80 символов заголовка.
    seen_events, seen_titles, unique = set(), set(), []
    for it in raw_items:
        event_key = f"{ticker}_{it.get('event', '')}" if it.get("event") else ""
        norm = re.sub(r'[^\w\s]', '', it["title"].lower())[:80].strip()
...
{{"explains_movement": true/false, "weight": число, "summary": "одно предложение"}}"""
    ...
    if blocking_found and is_long:
        ...
        filter_status = "WEAK" if is_htf_contrarian else "CONFIRMED"
    elif final_weight >= -2:
        filter_status = "WEAK" if is_htf_contrarian else "CONFIRMED"
    elif final_weight >= -5:
        filter_status = "WEAK"
...
MTF тренды: {mtf_str if mtf_str else "нет данных"}

СВЕЖИЕ НОВОСТИ ПО ИНСТРУМЕНТУ:
{news_lines}
...
Отвечай на русском, коротко и конкретно. Без воды."""
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("mode_"):
        ...
        # AI ревью — параллельно грузим новости и запускаем анализ
        try:
            _, sector = "", ""
