...
async def cmd_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not long_sigs and not short_sigs:
        ...
        extra = ""
        if watch_sigs:
            best = max(watch_sigs, key=lambda x: x["tech_score"])
    ...
    try:
        ...
        decision_log = [
            ...
            {"gate": "AI новостной фон", "result": filter_status,
             "detail": f"confidence={news_ai.get('confidence', 0)}%, weight={news_ai.get('event_weight', 0):+d} - {news_ai.get('summary', '')}".strip()},
        ]
