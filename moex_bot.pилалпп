...
class _AIRateLimiter:
    """Prostoy sinkhronnyy rate-limiter dlya AI-vyzovov (Gemini/Groq).
    Delayet sloty po vremeni, chtoby ne peregruzhat API.
    """
...
LIQUID_TICKERS = {
    ...
    "CHMF", "NLMK", "YDEX", "MGNT", "FIVE", "VTBR", "MOEX", "TCSG",
...
async def _build_diagnostics_report() -> list[str]:
    ...
    if OPENROUTER_API_KEY and openrouter_client:
        try:
            test_resp = openrouter_client.post(
                "https://openrouter.ai/api/v1/chat/completions",
                json={"model": "meta-llama/llama-3.3-70b-instruct", "messages": [{"role": "user", "content": "test"}], "max_tokens": 5},
                timeout=10,
            )
            if test_resp.status_code == 200:
                status["✅"].append(f"🔌 OpenRouter AI: подключен, ответ есть")
            else:
                status["⚠️"].append(f"🔌 OpenRouter AI: HTTP {test_resp.status_code}")
        except Exception as e:
            status["❌"].append(f"🔌 OpenRouter AI: ошибка - {str(e)[:80]}")
    else:
