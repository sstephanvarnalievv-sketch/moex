...
_last_weekend_briefing_sent: set = set()
...
async def run_weekend_briefing(app):
    ...
    """
    global _last_weekend_briefing_sent
    ...
    # Дедуп по часу, чтобы не слать один и тот же брифинг заново каждый час,
    # но всё же обновлять, если появилось что-то новое (новый сектор/новость).
    sig_keys = {f"sector_{s}_{m.get('reason','')[:40]}" for s, m in strong_mods.items()}
    sig_keys |= {f"macro_{it['title'][:40]}" for it in macro_news}
    sig_keys |= {f"company_{t}_{it.get('event', it['title'][:40])}" for t, it in company_facts}
    new_keys = sig_keys - _last_weekend_briefing_sent
    if not new_keys:
        return
    _last_weekend_briefing_sent |= sig_keys
    if len(_last_weekend_briefing_sent) > 200:
        _last_weekend_briefing_sent = set(list(_last_weekend_briefing_sent)[-100:])
